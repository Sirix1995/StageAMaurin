...
public class X3DTransformIO extends ObjectBase {
	...
	protected Node doImportImpl(Attributes atts) {
		...
		if (valueString!=null)
			newTransform.setX3dTranslation(Util.splitStringToTuple3f(new Vector3f(), valueString));

		valueString = atts.getValue("rotation");
		if (valueString!=null)
			newTransform.setX3dRotation(Util.splitStringToTuple4f(new Vector4f(), valueString));
		
		valueString = atts.getValue("scale");
		if (valueString!=null)
			newTransform.setX3dScale(Util.splitStringToTuple3f(new Vector3f(), valueString));
		
		valueString = atts.getValue("scaleOrientation");
		if (valueString!=null)
			newTransform.setX3dScaleOrientation(Util.splitStringToTuple4f(new Vector4f(), valueString));
		
		valueString = atts.getValue("center");
		if (valueString!=null)
			newTransform.setX3dCenter(Util.splitStringToTuple3f(new Point3f(), valueString));
		
		valueString = atts.getValue("bboxCenter");
		if (valueString!=null)
			newTransform.setX3dBboxCenter(Util.splitStringToTuple3f(new Point3f(), valueString));
		
		valueString = atts.getValue("bboxSize");
		if (valueString!=null) 
			newTransform.setX3dBboxSize(Util.splitStringToTuple3f(new Vector3f(), valueString));
	...
	public static Element handleTransformation(Matrix4d transMatrix, boolean isAxis, Node node, GraphState gs) {
		...
		//TODO: scaleOrientation/shear fehlt
		
		// set attributes except they have standard values
		if (!trans.equals(new Vector3f(0, 0, 0)))
			element.setAttribute("translation", trans.x + " " + trans.z + " " + -trans.y);
		if (arot.angle != 0)
			element.setAttribute("rotation" , arot.x + " " + arot.z + " " + -arot.y + " " + arot.angle);
		if (scale != 1)
