...
public abstract class CharTreeAttribute extends CharAttribute
{
	...
	{
		final ObjectMap<CharTreeAttribute.Listener> listeners;
		...
		void fire (GraphState gs, Object o, CharTreeAttribute.Listener l)
		{
			...
			if (!queueEvents)
			{
				...
				try
				{
					while (!pendingEvents.isEmpty ())
					{
						...
						{
							l = (CharTreeAttribute.Listener) queue.get (i);
	...
	final class Listener implements AttributeChangeListener
	{
		...
		CharTreeAttribute getAttribute ()
		{
			return CharTreeAttribute.this;
		}
	...
	protected abstract char derive (Object object, boolean asNode, char parentValue,
									 GraphState gs);
	...
	public char getParentValue (Object object, boolean asNode,
								 GraphState gs)
	{
		...
		{
			gs.moveToPreviousInstance ();
			char v = getDerived (gs.getInstancingPath ().getObject (i - 1), !asNode,
								  gs);
			gs.moveToNextInstance ();
			return v;
		}
