...
class Face {
	...
	public double getArea() {
		...
		Point3d p1 = he1.head().pnt;
		Point3d p2 = he2.head().pnt;

		double a = Math.sqrt(
				Math.pow(p0.x-p2.x, 2) + 
				Math.pow(p0.y-p2.y, 2) + 
				Math.pow(p0.z-p2.z, 2));
		double b = Math.sqrt(
	...
	void checkConsistency() {
		...
		if (numVerts < 3) {
			System.out.println("degenerate face: " + getVertexString());
		}
		do {
			HalfEdge hedgeOpp = hedge.getOpposite();
			if (hedgeOpp == null) {
				System.out.println("face " + getVertexString() + ": "
		...
		if (numv != numVerts) {
			System.out.println("face " + getVertexString() + " numVerts="
					+ numVerts + " should be " + numv);
		}
	...
	private void computeNormalAndCentroid() {
		...
		planeOffset = normal.dot(new Vector3d(centroid.x, centroid.y,
				centroid.z));
		int numv = 0;
		HalfEdge he = he0;
		do {
