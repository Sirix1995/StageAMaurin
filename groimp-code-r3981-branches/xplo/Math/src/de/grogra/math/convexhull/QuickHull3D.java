...
public class QuickHull3D {
		...
		for (Iterator<HalfEdge> it = horizon.iterator(); it.hasNext();) {
			...
			if (debug) {
				System.out.println("new face: "
						+ hedgeSide.face.getVertexString());
			}
	...
	protected void addPointToHull(Vertex eyeVtx) {
		...
		if (debug) {
			...
			System.out.println(" which is "
					+ eyeVtx.face.distanceToPlane(eyeVtx.pnt) + " above face "
		...
		for (int i = 0; i < numPoints; i++) {
			...
			for (Iterator<Face> it = faces.iterator(); it.hasNext();) {
				...
				if (face.mark == Face.VISIBLE) {
					...
					if (dist > pointTol) {
						if (ps != null) {
							ps.println("Point " + i + " " + dist
									+ " above face " + face.getVertexString());
						}
						return false;
					}
				}
	...
	protected void createInitialSimplex() throws IllegalArgumentException {
		...
		double maxDist = 0;
		Vector3d v2 = new Vector3d(vtx[2].pnt.x, vtx[2].pnt.y, vtx[2].pnt.z);
		double d0 = v2.dot(nrml);
		for (int i = 0; i < numPoints; i++) {
			Vector3d vi = new Vector3d(pointBuffer[i].pnt.x,
					pointBuffer[i].pnt.y, pointBuffer[i].pnt.z);
			double dist = Math.abs(vi.dot(nrml) - d0);
		...
		Vector3d v3 = new Vector3d(vtx[3].pnt.x, vtx[3].pnt.y, vtx[3].pnt.z);
		if (v3.dot(nrml) - d0 < 0) {
	...
	private boolean doAdjacentMerge(Face face, int mergeType) {
		...
		do {
			...
			if (merge) {
				if (debug) {
					System.out.println("  merging " + face.getVertexString()
							+ "  and  " + oppFace.getVertexString());
				}
	...
	public double getSurfaceArea() {
		...
		return getAreaOfTriangultion(data);
	}
	 ...
	 * @return area 
	 */
	private double getAreaOfTriangultion (float[] pointlist) {
		// check input data
		if(pointlist.length<9) {
			System.out.println("Error in QuickHull.getAreaOfTriangultion: Number of points must be at least three. I switched to the default leaf.");
			return 0;
		}
		if(pointlist.length%3!=0) {
			System.out.println("Error in QuickHull.getAreaOfTriangultion: Number of points must be a multiple of three. I switched to the default leaf.");
			return 0;
		}

		double area = 0;
		// triangulate data
		for(int i=0; i<=pointlist.length/3-3; i++) {
			double a = Math.sqrt(
					Math.pow((pointlist[i*3+0]-pointlist[i*3+6]), 2) + 
					Math.pow((pointlist[i*3+1]-pointlist[i*3+7]), 2) + 
					Math.pow((pointlist[i*3+2]-pointlist[i*3+8]), 2));
			double b = Math.sqrt(
					Math.pow((pointlist[i*3+6]-pointlist[i*3+3]), 2) + 
					Math.pow((pointlist[i*3+7]-pointlist[i*3+4]), 2) + 
					Math.pow((pointlist[i*3+8]-pointlist[i*3+5]), 2));
			double c = Math.sqrt(
	...
	public Vector<Face> getFacesVector() {
		return faces;
	}
	...
	public Point3d[] getPointBuffer() {
		Point3d[] vtxs = new Point3d[numPoints];
		for (int i = 0; i < numPoints; i++) {
			vtxs[i] = pointBuffer[i].pnt;
		}
		return vtxs;
	}
	 ...
	 * based on: http://stackoverflow.com/questions/9325303/centroid-of-convex-polyhedron/9325812#9325812
	 * The centroid = SUM(pos*volume)/SUM(volume) when you split the part into finite volumes 
	 * each with a location pos and volume value volume.
	 * This is precisely the calculation done for finding the center of gravity of a composite part.
	 * 
	 * @return point3d of the centroid
	 */
	public Point3d getCentroid() {
		// Compute the average of the facial centroids.
		// This gives an arbitrary point inside the polyhedron.
		Vector3d avgPoint = new Vector3d(0, 0, 0);
		Vector<Face> faces = getFacesVector();
		...
		avgPoint.x = avgPoint.x /faces.size();
		avgPoint.y = avgPoint.y /faces.size();
		avgPoint.z = avgPoint.z /faces.size();
		...
		for (int i = 0; i < faces.size(); i++) {
			...
			Point3d avgToCentroid = new Point3d();
			face.computeCentroid(avgToCentroid);
			...
			Vector3d avgToCentroidV = new Vector3d(avgToCentroid);
			float distance = (float)avgToCentroidV.dot(normal);

			// Finds the volume of the pyramid using V = 1/3 * B * h
			// where:   B = area of the pyramid base.
			//          h = pyramid height.
			float pyramidVolume = (float)face.getArea() * distance / 3f;

			// Centroid of a pyramid is 1/4 of the height up from the base.
			// Using 3/4 here because vector is travelling 'down' the pyramid.
			avgToCentroidV.scale(0.75f);
			avgToCentroidV.add(avgPoint);
			// avgToCentroid is now the centroid of the pyramid.

			// Weight it by the volume of the pyramid.
			avgToCentroidV.scale(pyramidVolume);

			//mh: added this line, because i think it is missing in the original code
			centroid.add(avgToCentroidV);
			volume += pyramidVolume;
		}

		// Average the weighted sum of pyramid centroids.
		centroid.x = centroid.x / volume;
		centroid.y = centroid.x / volume;
		centroid.z = centroid.x / volume;
		return new Point3d(centroid.x,centroid.y, centroid.z);
	}
