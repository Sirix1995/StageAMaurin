 ...
 * RH20120221 fixed bug in setSize(int, int)
 */
 ...
 */
	...
	if (this.nRow != nRow || this.nCol != nCol)
	{
		...
		// check for padding
		if (oldnCol == nCol) {
		    // no padding needed, can copy as one big block of data
		    System.arraycopy(oldData, 0, elementData, 0, min(oldSize, size));
		} else {
			// number of columns to copy
			int cols = min(nCol, oldnCol);
			// number of rows to copy
			int rows = min(nRow, oldnRow);
			// copy each partial row
		    for (int i = 0; i < rows; i++)
		    	System.arraycopy(oldData, i*oldnCol, elementData, i*nCol, cols);
		}
	}
    }
