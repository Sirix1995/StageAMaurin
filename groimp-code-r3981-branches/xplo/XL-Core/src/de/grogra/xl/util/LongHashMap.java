...
public class LongHashMap<V> implements Iterable<LongHashMap.Entry<V>>
{
	...
	public LongHashMap (int capacity, float loadFactor)
	{
        ...
        table = (Entry<V>[]) new LongHashMap.Entry[c];
        lengthM1 = c - 1;
		size = 0;
	}
...
;
        ...
        {
            Entry<V>[] t = new LongHashMap.Entry[(lengthM1 + 1) << 1];
            Entry<V>[] old = table;
            int tlM1 = t.length - 1;
...
;
	...
	public Iterator<Entry<V>> iterator ()
	{
		return new Iterator<Entry<V>> ()
		{
			...
			public void remove ()
			{
				LongHashMap.this.remove (current);
			}
		};
	}
