...
/**
 * A <code>FloatList</code> represents a list of <code>float</code> values.
...
public class FloatList
	implements
Cloneable, java.io.Serializable, de.grogra.xl.lang.FloatConsumer,
de.grogra.xl.lang.VoidToFloatGenerator
{
	...
	/**
	 * Constructs a new <code>FloatList</code> with a given initial capacity.
	 *
	 * @param capacity the initial capacity
	 */
	public FloatList (int capacity)
	{
		elements = new float[Math.max (capacity, 8)];
	}


	/**
	 * Constructs a new <code>FloatList</code>.
	 */
	public FloatList ()
	{
		this (16);
	...
	/**
	 * Constructs a new <code>FloatList</code> whose elements are
	...
	public Object clone ()
	{
		try
		{
			FloatList  c = (FloatList ) super.clone ();
			c.elements = new float[Math.max (size, 8)];
	...
	public void ensureCapacity (int capacity)
	{
		...
		{
			float[] a = new float[Math.max (capacity,
											elements.length * 2)];
	...
	public void trimToSize ()
	{
		if (size < elements.length)
		{
			...
			{
				arraycopy (elements, 0, elements = new float[size], 0, size);
			}
		}
	}
	 ...
	 * yet contained as reported by {@link #contains(float)}.
	 *
	 * @param o the value to add
	 */
	 ...
	 * @return this list
	 */
	public FloatList  addAll (FloatList
		v)
	{
	...
	{
		return (index < size) ? (float) elements[index] : 0;
	}
	...
	{
		return (index > 0) ? (float) elements[size - index] : 0;
	}
	...
	public static void arraycopy (float[] src, int srcIndex,
	...
	public static void clear (float[] array, int index, int length)
	{
		...
		else
		{
			while (length > 0)
			{
				...
				arraycopy (FLOAT_NULL, 0, array, index, n);
				length -= n;
				index += n;
			}
		}
	...
	public void evaluateFloat (de.grogra.xl.lang.FloatConsumer cons)
	{
		for (int i = 0; i < size; i++)
		{
			cons.consume ((float) elements[i]);
		}
	}

	/**
	 * This method is an alias for {@link #evaluateFloat}.
	 */
	...
	{
		s.defaultReadObject ();
		float[] a = elements = new float[size];
		for (int i = 0; i < size; i++)
		{
			a[i] = s.readFloat ();
		}
