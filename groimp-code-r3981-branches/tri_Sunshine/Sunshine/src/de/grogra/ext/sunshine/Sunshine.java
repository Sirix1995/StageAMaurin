...
public class Sunshine extends de.grogra.imp.Renderer implements Options, Runnable
{
	public static final EnumerationType RAYPROCESSOR = new EnumerationType (
			"rayprocessor", SunshinePlugin.I18N, new String[] {"standard", "pathtracer", "bidirectional", "spectral"},
			new Class[] {
					SunshineStandardRaytracer.class, 
					SunshinePathtracer.class, 
					SunshineBidirPathtracer.class,
					SunshineSpectralPT.class
			}, Type.CLASS);
	...
	public void run ()
	{
		...
		ObjectHandler oh = new ObjectHandler(cv.getSize(), cv.getObjects(), cv.getTriangleCount(), cv.getImageCache() );
		...
		SunshineSceneVisitor visitor = new SunshineSceneVisitor( 
				view3D.getWorkbench(), view3D.getGraph(), gs, view3D, oh, this, 
		...
		if( cv.getSize() > 0 )
		{
			...
			renderer = (SunshineRaytracer) getClassOption("rayprocessor", new SunshineStandardRaytracer());
			renderer.initialize(this, visitor, oh, width, height, cv.getSize(), cv.getObjects(), cv.getTriangleCount());
	...
	{
		Image image = new Image(width, height, mode);
