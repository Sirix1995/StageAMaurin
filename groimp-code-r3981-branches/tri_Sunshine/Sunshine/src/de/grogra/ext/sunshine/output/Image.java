...
import de.grogra.ext.sunshine.VisibleColor;
...
//		image = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);
		...
		try {
			...
			for(int y = 0; y < height ; y++)
			{
				...
				/*	output.applyValues(values);
					...
					minFloat = Math.min(minFloat, values[0]);
					minFloat = Math.min(minFloat, values[1]);
					minFloat = Math.min(minFloat, values[2]);
					minFloat = Math.min(minFloat, values[3]);					
					
					maxFloat = Math.max(maxFloat, values[0]);
					maxFloat = Math.max(maxFloat, values[1]);
					maxFloat = Math.max(maxFloat, values[2]);
					maxFloat = Math.max(maxFloat, values[3]);
			...
			/*		for(int y = 0; y < height ; y++)
			{
				int lMin = 380;
				int lMax = 780;
				int deltaLamda = lMax - lMin;
				
				float red, green, blue, alpha;
			
				for (int x = 0; x < width; x++) 
				{	
					double lamda = (((double) x) / ((double) width) * deltaLamda) + lMin; // + 20;
					
					
					Color col = VisibleColor.wavelengthToColor(lamda);
					...
					alpha = ((float) col.getAlpha()) / 255.0f;
					
					hdrPixels[0][offset] = red;
					hdrPixels[1][offset] = green;
					hdrPixels[2][offset] = blue;
					hdrPixels[3][offset] = alpha;
					
					
					int lamda = (int) (((float) x) / ((float) width) * deltaLamda) + 20;
					
					Vector3f rgb		= new Vector3f(matchFunctionXYZ[lamda * 3 + 0],
												matchFunctionXYZ[lamda * 3 + 1],
												matchFunctionXYZ[lamda * 3 + 2]
														);
					...
					float minR 	= Math.min(0, rgb.x);
					float minG 	= Math.min(0, rgb.y);
					float minB 	= Math.min(0, rgb.z);
					
					float minV 	= Math.min(minR, minG);
					minV 		= -Math.min(minV, minB);
					
						
					if(minV > 0.0)
						rgb.add(new Vector3f(minV, minV, minV));
					else
						minV = 0;
					...
					hdrPixels[0][offset] = rgb.x / (maxFloat);
					hdrPixels[1][offset] = rgb.y / (maxFloat);
					hdrPixels[2][offset] = rgb.z / (maxFloat);
					hdrPixels[3][offset] = 1.0f;
					
					
					hdrPixels[0][offset] -= (minFloat);
					hdrPixels[1][offset] -= (minFloat);
					hdrPixels[2][offset] -= (minFloat);
					hdrPixels[3][offset] = 1.0f;
					
					hdrPixels[0][offset] /= (maxFloat);
					hdrPixels[1][offset] /= (maxFloat);
					hdrPixels[2][offset] /= (maxFloat);
					hdrPixels[3][offset] = 1.0f;
	...
	// 360 - 830
	final public static float[] matchFunctionXYZ = new float[] 
	                                                     	{                                                   
	                                                     		0.000000122200f, 0.000000013398f, 0.000000535027f,		// 360nm
	                                                     		...
	                                                     		0.000001553140f, 0.000000629700f, 0.000000000000f	//830
