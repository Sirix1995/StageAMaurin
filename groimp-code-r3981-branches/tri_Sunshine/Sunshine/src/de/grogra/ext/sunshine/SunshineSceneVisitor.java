...
public class SunshineSceneVisitor extends DisplayVisitor implements ProgressMonitor
{
	...
	public SunshineSceneVisitor(Workbench wb, Graph graph, GraphState gs, ViewConfig3D view,
	...
	protected void storeImage(ByteBuffer imageBB)
	{
		storeImage(imageBB, de.grogra.ext.sunshine.output.Image.OUTPUT_CLAMP);
	}
...
//					if(ior != null) System.out.println("iorRatio" + ((IOR)ior).getIndexOfRefraction() );
				...
				if(shape instanceof MeshNode)
				{
					...
					int[] index 	= p.getIndexData();		// every tri has 3 pointer (indices) on vertices
					
					if(index.length > 0)
					{
						float[] vertex 	= p.getVertexData();	// all unique vertices are stored as a sequence (may used by some tri)	
						float[] normal	= p.getNormalData();
						float[] texture	= p.getTextureData();
						
						int triCount	= index.length / 3;		// to obtain the number of tri with the number of indices
						
						sObject = new SunshineTriangles(true, vertex, index, normal, texture);
