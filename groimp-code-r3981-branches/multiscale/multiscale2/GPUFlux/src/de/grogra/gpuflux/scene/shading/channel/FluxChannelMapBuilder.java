...
public abstract class FluxChannelMapBuilder implements ChannelMapNodeVisitor {
	...
	{
		currentChannelNode = null; //this.defaultChannel;
		
		if( channelMap == null )
			channelMap = defaultChannel;
				
		if( channelMap != null &&channelMap instanceof ChannelMap )
			((ChannelMap)channelMap).accept(this);
	
		//assert currentChannelNode != null;
	...
	private void discretizeChannel(ChannelMap map)
	{
		...
		if( chachedMap == null )
		{
			...
			ChannelData src = ((ChannelMapInput) env.userObject).getUserData (map, null),
				sink = src.createSink (map);
			...
			// set shared source inputs 
			src.setFloat (Channel.IOR, env.iorRatio);
			src.setTuple3f (Channel.DPXDU, env.dpdu);
			src.setTuple3f (Channel.DPXDV, env.dpdv);
			
			// discretize the channel
			for( int x = 0 ; x < channelResolution ; x++ )
				for( int y = 0 ; y < channelResolution ; y++ )
				{
