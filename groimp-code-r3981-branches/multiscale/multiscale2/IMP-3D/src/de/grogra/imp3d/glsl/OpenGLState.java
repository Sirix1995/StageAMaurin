...
public class OpenGLState {
	...
	public void initLineCache(GLDisplay disp) {
		if(lineCache == null)
			lineCache = new LineSegmentizationCache(disp.getRenderGraphState(), 1);
	}
	...
	public static final int attachPoints[] = { GL.GL_COLOR_ATTACHMENT0_EXT,
			GL.GL_COLOR_ATTACHMENT1_EXT, GL.GL_COLOR_ATTACHMENT2_EXT,
			GL.GL_COLOR_ATTACHMENT3_EXT, GL.GL_COLOR_ATTACHMENT4_EXT,
			GL.GL_COLOR_ATTACHMENT5_EXT, GL.GL_COLOR_ATTACHMENT6_EXT,
...
//	}
	...
	private static final char[] stateToInternalState = { ALPHA_TEST_BIT,
			STENCIL_TEST_BIT, DEPTH_TEST_BIT, LIGHTING_BIT, TEXTURE_2D_BIT,
			BLEND_BIT, CULLING_BIT, FREE2_BIT };

	private static final int[] stateToGLState = { GL.GL_ALPHA_TEST,
			GL.GL_STENCIL_TEST, GL.GL_DEPTH_TEST, GL.GL_LIGHTING,
			GL.GL_TEXTURE_2D, GL.GL_BLEND, GL.GL_CULL_FACE, 0 };
	...
	public void setState(char state) {
		...
		for (char i = 0; i < 6; i++)
			if ((different & (1 << i)) > 0) {
				if ((state & (1 << i)) > 0)
					getGL().glEnable(stateToGLState[i]);
				else
					getGL().glDisable(stateToGLState[i]);
			}
		this.state = state;
	}
...
//	 MaterialConfiguration depthPeelingMat = new DepthPeelingMaterial();
...
//		case DEPTH_PEELIG_MATERIAL:
//			 return depthPeelingMat;
	...
	public void testFBO() {
		...
		if (GLSLDisplay.DEBUG) {
			...
			case GL.GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT_EXT:
				GLSLDisplay
						.printDebugInfoN("Framebuffer incomplete attachment");
				break;
			case GL.GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS_EXT:
				GLSLDisplay
						.printDebugInfoN("Framebuffer incomplete dimensions");
				break;
			case GL.GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER_EXT:
				GLSLDisplay
						.printDebugInfoN("Framebuffer incomplete draw buffer");
				break;
			...
			case GL.GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE_EXT:
				GLSLDisplay
						.printDebugInfoN("Framebuffer incomplete multisample");
				break;
			...
			case GL.GL_FRAMEBUFFER_INCOMPLETE_LAYER_COUNT_EXT:
				GLSLDisplay
						.printDebugInfoN("Framebuffer incomplete layer count");
				break;
			case GL.GL_FRAMEBUFFER_INCOMPLETE_LAYER_TARGETS_EXT:
				GLSLDisplay
						.printDebugInfoN("Framebuffer incomplete layer targets");
				break;
	...
	public void printGPUMemoryUsage(GLSLDisplay disp) {
		...
		tmp  = skyDiffuseCube.estimateSizeInByte() + 
				skyCube.estimateSizeInByte();
		memoryUsage += tmp;
...
//			gl.glColor3f(.5f, 1, 0.5f);
//			gl.glVertex3f(0,0,0);
//			gl.glVertex3f(paraLight[4].x, paraLight[4].y, paraLight[4].z);
//		} gl.glEnd();
