...
//		return new Color3f(c);
			...
			float scaleV, Shader s, int highlight, Matrix4d t)
	{
		...
		// draw the frustum //
		drawFrustumIrregularImpl (gl, uCount, topclosed, topRadii, baseClosed,
			baseRadii);
			...
			float[] topRadii, boolean baseClosed, float[] baseRadii)
	{
		...
		for (int u = 0; u <= uCount; u++)
		{
			...
			gl.glNormal3f (cosPhi, sinPhi, 0);
			gl.glTexCoord2f (phi / (float) (2.0f * Math.PI), 1);
			gl.glVertex3f ((float) (topRadii[u] * cosPhi),
				(float) (topRadii[u] * sinPhi), 1);
			gl.glTexCoord2f (phi / (float) (2.0f * Math.PI), 0);
			gl.glVertex3f ((float) (baseRadii[u] * cosPhi),
				(float) (baseRadii[u] * sinPhi), 0);
		}
		gl.glEnd ();
	...
	public void drawPrismRectangular(float y, float xPos, float xNeg,
			float zPos, float zNeg, int highlight, Matrix4d t) {
