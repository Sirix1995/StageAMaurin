...
/*
 * Modifications for GroIMP-integration made by Ole Kniemeyer 2006/07/23
 * Copyright (C) 2006 Lehrstuhl Grafische Systeme, BTU Cottbus
 */
...
public class BufferIORequest extends WorkRequest
{
		...
		try
		{
			...
			try
			{
				...
				if(out != null)
				{
					...
					// 2006/07/23: modified by Ole Kniemeyer
					write(buffer,savePath,out);
	...
	// 2006/07/23: path-argument added by Ole Kniemeyer
	//{{{ write() method
	private void write(Buffer buffer, String path, OutputStream _out)
		throws IOException
	{
		...
		try
		{
			...
			// 2006/07/23: added by Ole Kniemeyer
			vfs.setEncoding (path, encoding);
		...
		{
			if(out != null)
				// 2007/05/04: changed by Ole Kniemeyer
				{out.flush();out.close();}
			else
				_out.close();
		}
