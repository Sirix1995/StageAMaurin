...
class Reflect 
{
	...
	*/
        ...
        throws ReflectError, UtilEvalError
	{
    	try
    	{
    		return resolveExpectedJavaMethodImpl
    			( bcm, clas, object, name, args, onlyStatic);
    	}
    	catch (ReflectError e)
    	{
    		return resolveExpectedJavaMethodImpl
    			( bcm, clas, object, name, substituteArgs( args, e ), onlyStatic);
    	}
	}

    private static Method resolveExpectedJavaMethodImpl(
		BshClassManager bcm, Class clas, Object object, 
		String name, Object[] args, boolean onlyStatic )
        throws ReflectError, UtilEvalError
    {
...
/*
	...
	*/
	...
	// 2005/08/17: added by Ole Kniemeyer
	private static Object[] substituteArgs( Object[] args, ReflectError e)
		throws ReflectError
	{
		if (args != null)
		{
			for (int i = 0; i < args.length; i++)
			{
				if (args[i] instanceof org.gjt.sp.jedit.View)
				{
					args[i] = ((org.gjt.sp.jedit.View) args[i]).getFrame ();
					return args;
				}
			}
		}
	...
	*/
    	...
    	throws ReflectError, InvocationTargetException
    {
    	try
    	{
    		return constructObjectImpl( clas, args );
    	}
    	catch (ReflectError e)
    	{
    		return constructObjectImpl( clas, substituteArgs( args, e ) );
    	}
    }

	// 2005/08/17: previous method added, name of the following changed
    // by Ole Kniemeyer

    static Object constructObjectImpl( Class clas, Object[] args )
        throws ReflectError, InvocationTargetException
    {
		if ( clas.isInterface() )
			throw new ReflectError(
