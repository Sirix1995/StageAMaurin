...
public abstract class DoubleTreeAttribute extends DoubleAttribute
{
	...
	{
		final ObjectMap<DoubleTreeAttribute.Listener> listeners;
		...
		void fire (GraphState gs, Object o, DoubleTreeAttribute.Listener l)
		{
			...
			if (!queueEvents)
			{
				...
				try
				{
					while (!pendingEvents.isEmpty ())
					{
						...
						{
							l = (DoubleTreeAttribute.Listener) queue.get (i);
	...
	final class Listener implements AttributeChangeListener
	{
		...
		DoubleTreeAttribute getAttribute ()
		{
			return DoubleTreeAttribute.this;
		}
	...
	protected abstract double derive (Object object, boolean asNode, double parentValue,
									 GraphState gs);
	...
	public double getParentValue (Object object, boolean asNode,
								 GraphState gs)
	{
		...
		{
			gs.moveToPreviousInstance ();
			double v = getDerived (gs.getInstancingPath ().getObject (i - 1), !asNode,
								  gs);
			gs.moveToNextInstance ();
			return v;
		}
