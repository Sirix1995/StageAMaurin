...
scaleDeclaration[AST mods]
	:	insert[mods]
		SCALE^ id:IDENT params:moduleParameterDeclarationList[id]! observerExtendsClause
		(	
			(	b:moduleCtorBlock!
			)?
		)?
	...
	:
		{
			...
			ASTWithToken o = new ASTWithToken(IDENT, "de.grogra.rgg.MSObservable");
			Token t1 = new Token(EXTENDS, "extends");
			Token t2 = new Token(IDENT, "de.grogra.rgg.MSObservable");
			e.token = t1;
			o.token = t2;
			astFactory.makeASTRoot(p, e);
			astFactory.addASTChild(p, o);
			#observerExtendsClause = p.root;
		}
	;
...
productionStatement[AST prev]
	...
	|	e2:produceEdgeOp! lc:LCLIQUE edgeNode[#e2, #lc] (edgeNode[#[EMPTY],#lc])* RCLIQUE
	|	lc2:LCLIQUE (edgeNode[#[EMPTY], #lc2])* RCLIQUE
	;
...
productionStatementsWithoutClique
	...
	:	(	s:productionStatementWithoutClique[prev]
			{
				if (#s != null)
				{
					prev = #s;
...
productionStatementWithoutClique[AST prev]
	:	script
	|	id:IDENT lbl:COLON^
		(	(controlStatement[true] | productionBlock)
			setType[#lbl, LABELED_STATEMENT]
		|	n:node[#[EMPTY], #id, prev]! {#productionStatementWithoutClique = #n;}
		)
	|	controlStatement[true]
	|	THROW^ primaryExpressionNode
	|	c:COMMA setType[#c, SEPARATE]
	|	lb:LBRACK^ productionStatementsWithoutClique RBRACK! setType[#lb, TREE]
	|   BREAK^ (options {warnWhenFollowAmbig = false;}: IDENT)?
	|   CONTINUE^ (options {warnWhenFollowAmbig = false;}: IDENT)?
	|	node[#[EMPTY], null, prev]
	|	e:produceEdgeOp! edgeNode[#e, prev]
