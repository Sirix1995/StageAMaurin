...
public class LightDistribution extends ShareableBase
{
// enh:sco SCOType
		...
		protected void setInt (Object o, int id, int value)
		{
			switch (id)
			{
				case Type.SUPER_FIELD_COUNT + 0:
					((LightDistribution) o).width = (int) value;
					return;
				case Type.SUPER_FIELD_COUNT + 1:
					((LightDistribution) o).height = (int) value;
					return;
			}
...
//enh:end
	...
	public void setDistribution( double [] [] lipdf )
	{
		...
		for( int y = 0 ; y < height; y++ )
			for( int x = 0 ; x < width; x++)
			{
				...
				float theta_0 = (float) ((x / (float)width) * Math2.M_PI);
				float phi_0 = (float) ((y / (float)height) * 2 * Math2.M_PI - Math2.M_PI);
				float theta_1 = (float) (((x+1) / (float)width) * Math2.M_PI);
				float phi_1 = (float) (((y+1) / (float)height) * 2 * Math2.M_PI - Math2.M_PI);
	...
	public double map2direction(Vector3f outDirection, Tuple2d inPoint) {
		...
		float theta = (float) (u * Math2.M_PI);
		float phi = (float) (v * 2 * Math2.M_PI - Math2.M_PI);
		...
		return pdf_interval * (width*height) / (2*Math2.M_PI*Math2.M_PI*sint);
	}
	
	public static Vector3f map2cartesian( float r1, float r2 )
	{
		// set out direction
		float theta = (float) (r1 * Math2.M_PI);
		float phi = (float) (r2 * 2 * Math2.M_PI - Math2.M_PI);
