	...
	int root )
{
	...
	bvhTraceSensor( DEBUG_ARG, irradiance_buffer, detectors, sensitivityCurves, measurementBits, irradiance, spectrum, length, r, sensors, bvh, root );
#else
	computeSensorIntersects( DEBUG_ARG , irradiance_buffer, detectors, sensitivityCurves, measurementBits, irradiance, spectrum, 0, ns, sensors, length, r );
#endif
