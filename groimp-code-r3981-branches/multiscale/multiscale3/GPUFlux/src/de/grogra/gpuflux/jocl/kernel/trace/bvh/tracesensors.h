	...
	int root)
{
	...
	while(nodeAddr!=ENTRYPOINT_SENTINEL)
	{
		...
		if(nodeAddr<0)
		{
			...
			//---------------------------------------------------------
			computeSensorIntersects( DEBUG_ARG, irradiance_buffer, detectors, sensitivityCurves, measurementBits, irradiance, spectrum, sensorAddr, sensorCount, sensors, length, r );
