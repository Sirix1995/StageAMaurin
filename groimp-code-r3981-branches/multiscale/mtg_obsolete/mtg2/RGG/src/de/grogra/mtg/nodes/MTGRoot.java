...
public class MTGRoot extends MTGNode implements Renderable,Polygonizable
{
	...
	public void plantFrame(int scale, float dist) throws MTGPlantFrameException
	{
		try
		{
			...
			//refresh mesh - mesh recomputation on demand
			polygons=null;
		}
		catch(Throwable t)
		{
	 ...
	 * @throws MTGPlantFrameException
	 */
	private void computeGeometryStandard(int coordType, int scale) throws MTGPlantFrameException
	{
	 ...
	 * @param dirp
	 */
	private void setElementLengthDirectionOrder(MTGBranchElement element, float length, Vector3d dirp)
	{
	 ...
	 * @throws MTGPlantFrameException
	 */
	private void computeBranchLengthsStandard(int branchIndex) throws MTGPlantFrameException
	{
	 ...
	 * @throws MTGPlantFrameException
	 */
	private boolean verifyTriangleReference() throws MTGPlantFrameException
	{
	 ...
	 * @throws MTGPlantFrameException
	 */
	private boolean verifyCartesianReference() throws MTGPlantFrameException
	{
		boolean x,y,z;
		x=y=z=false;
	 ...
	 * @throws MTGPlantFrameException
	 */
	private void lookForBranches(int originIndex,int scale, int order) throws MTGPlantFrameException
	{	
	 ...
	 * @throws MTGPlantFrameException 
	 */
	private int getPrefix(int nodeIndex) throws MTGPlantFrameException
	{
		MTGNode node = getNode(nodeIndex);
		if(node==null)
	...
	private Vector3d minVector3d(Vector3d a, Vector3d b)
	{
	...
	private Vector3d maxVector3d(Vector3d a, Vector3d b)
	{
		double x = (a.x>=b.x)?a.x:b.x;
		double y = (a.y>=b.y)?a.y:b.y;
		double z = (a.z>=b.z)?a.z:b.z;
						...
						if(origin!=null)
						{
							...
							if(j!=branch.getElementCount()-1)
							{
								...
								if(origin2!=null)
								{
									...
									if(branch.getScale()==0)
										rs.drawLine(pt, pt2, blueLight, 0, null);
									else if(branch.getScale()==1)
										rs.drawLine(pt, pt2, redDark, 0, null);
									else if(branch.getScale()==2)
										rs.drawLine(pt, pt2, blueDark, 0, null);
									else if(branch.getScale()==3)
										rs.drawLine(pt, pt2, greenDark, 0, null);
									else if(branch.getScale()==4)
										rs.drawLine(pt, pt2, redLight, 0, null);
