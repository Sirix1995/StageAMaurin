...
//		gl.glPushMatrix();
//		gl.glLoadMatrixd(disp.toGLMatrix(worldToView), 0);
		...
		if(disp.isOptionLighting()) {
			...
			while(it.hasNext()) {
				...
				if(disp.isOptionShowDiffuseSkyLight() || !(light.getLight() instanceof Sky) )
				{
					...
					if(light.getLight().getLightType() != de.grogra.ray.physics.Light.NO_LIGHT)
						gotOneLight = true;
				}
			}
		}
...
//		gl.glPopMatrix();
	...
	private LightPos getDefaultLight(GLSLDisplay disp) {
			...
			if(defaultLightPos == null)
				defaultLightPos = new LightPos(defaultLight, defaultLightToWorld);
			else
				defaultLightPos.setLightPos(defaultLightToWorld);
