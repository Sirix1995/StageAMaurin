...
//		return new Color3f(c);
	...
	{
		Object scaleVisibleObj = this.getView().getWorkbench().getProperty("scale visible");
		if(scaleVisibleObj!=null)
		{
			return (boolean[])scaleVisibleObj;
		}
		return null;
	}
	...
	{
		Object scaleChangedObj = getView().getWorkbench().getProperty("scale changed");
		if(scaleChangedObj!=null)
		{
			return ((Boolean)scaleChangedObj).booleanValue();
		}
		return true;
	}
	
	public void setScaleChanged(boolean changed)
	{
		getView().getWorkbench().setProperty("scale changed", new Boolean(changed));
	}
}
