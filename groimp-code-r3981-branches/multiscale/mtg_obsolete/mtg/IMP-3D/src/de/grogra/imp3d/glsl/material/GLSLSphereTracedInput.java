...
public class GLSLSphereTracedInput extends GLSLChannelMap {
	...
	public Result generate(ChannelMap inp, MaterialConfiguration cs,
		...
		if(pos == null)
			pos = cs.registerNewTmpVar(Result.ET_VEC3, "(gl_TextureMatrix[2] * vec4(normalize(vec3(TexUnit2, -1.0)), 0.0)).xyz;");
		switch (channel) {
		case Channel.U:
			if(uv == null)
				uv = cs.registerNewTmpVar(Result.ET_VEC2, "vec2(atan("+pos+".y, -"+pos+".x) * 0.5, " +
								  "acos(-"+pos+".z) ) / 3.14159265358979323846264 + vec2(0.5, 0.0);");
