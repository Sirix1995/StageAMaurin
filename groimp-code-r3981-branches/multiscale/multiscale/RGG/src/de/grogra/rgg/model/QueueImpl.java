...
class QueueImpl extends GraphQueue implements ObjectConsumer
{
	...
	//yong 3 Apr 2012 - macro connection
	static final int CONNECT_MACRO = EMBED_INTERPRETIVE + 1;

	//static final int EXECUTE = EMBED_INTERPRETIVE + 1;
	static final int EXECUTE = CONNECT_MACRO + 1;
	//yong 3 Apr 2012 - macro connection - END
...
// generated
		...
		//yong 3 apr 2012 - macro connection
		setItemSize (CONNECT_MACRO, 0, 1, 2, 0, 0, 0);
...
// generated
	...
	//yong 3 Apr 2012 - macro connections
	@Override
	public void connectMacro (Object source, Object target, int edges)
	{
		if (edges == 0)
		{
			return;
		...
		{
			makePersistentQueue.makePersistent (targetNode);
		}
		
		Node srcNode = (Node) source;
		if (srcNode.getId () < 0)
		{
			makePersistentQueue.makePersistent (srcNode);
		}
		
		writeItem (CONNECT_MACRO);
		writeLong (((Node) source).getId ());
		writeLong (((Node) target).getId ());
		writeInt (edges);
	}
	//yong 3 Apr 2012 - macro connections - END
				...
				throws IOException, RuntimeModelException
			{
				switch (item & ITEM_MASK)
				{
					...
					case CONNECT_MACRO:
						...
						Node src = manager.getObject (readLong ());
						Node tgt = manager.getObject (readLong ());
						int edge = readInt();
						boolean hasMoreMacro=true;
						Node srcParent = null;
						Node tgtParent = null;
						while(hasMoreMacro)
						{
							srcParent = src.getRefinementParent();
							tgtParent = tgt.getRefinementParent();
							
							if((srcParent!=null)&&(tgtParent!=null))
							{
								srcParent.addEdgeBitsTo(tgtParent, edge, transaction);
								src=srcParent;
								tgt=tgtParent;
							}
							else if((srcParent==null)&&(tgtParent!=null))
							{
								src.addEdgeBitsTo(tgtParent, edge, transaction);
								tgt=tgtParent;
							}
							else
							{
	...
	public void consume (Object v)
	{
		...
		//yong 3 apr 2012 - some comments
		// - 'half-edge' mapping performed here.
		// - searching to see if 'other' node is also rewritten. if so, perform connection with 'other's rewritten' node.
		...
		// if 'other' node is not rewritten, perform connection with node in G- graph.
		if (!found)
		{
		...
		//yong 3 apr 2012 - some comments - ned
	}
	...
	public boolean process (int[] segs) throws RuntimeModelException
	{
		...
		connEdges.clear ();
		//yong 3 apr 2012 - some comments
		//fill up inConnTrans and outConnTrans from queue entry
		boolean modified = super.process (segs);
		
		//begin to check for half-edge connection matching
		
		/*
		 * Outgoing connection transformation half-edge matching 
		 */
		//set otherConnTransMap to incoming direction
		otherConnTransMap = inConnTrans;
		//set curConnTransOut to true
		curConnTransOut = true;
		//check half-edge connection matching for outgoing connection transformations
		...
		/*
		 * Incoming connection transformation half-edge matching 
		 */
		//set otherConnTransMap to outgoing direction
		otherConnTransMap = outConnTrans;
		//set curConnTransOut to false
		curConnTransOut = false;
		//check half-edge connection matching for incoming connection transformations
		...
		//end check for half-edge connection matching
		
		//execute real connection transformations
		if (!connNodes.isEmpty ())
		{
			...
			while (!connNodes.isEmpty ())
			{
				...
				//yong 3 apr 2012 - modified for macro connection
				Node s = (Node) connNodes.pop ();
				int edge = connEdges.pop();
				//((Node) connNodes.pop ()).addEdgeBitsTo (t, connEdges.pop (), transaction);
				s.addEdgeBitsTo (t, edge, transaction);
				//yong 3 apr 2012 - modified for macro connection - END
				
				//yong 3 Apr 2012 - add entries to 'ConnectMacro' queue
				GraphQueue connectMacroQueue = graph.getQueues().getQueue(GraphQueue.CONNECT_MACRO_DESCRIPTOR);
				connectMacroQueue.connectMacro(s, t, edge);
				//yong 3 Apr 2012 - end
			}
		}
		inConnTrans.clear ();
