...
public class RGG extends Node implements Selectable, RegistryContext
{
	...
	protected void reset ()
	{
		...
		//yong - 23 mar 2012 - adding type graph
		TypeRoot typeRoot = new TypeRoot();
		typeRoot.addEdgeBitsTo(new AxiomType(), Graph.SUCCESSOR_EDGE, xa);
		rr.addEdgeBitsTo(typeRoot, Graph.SUCCESSOR_EDGE,xa);
		//yong - 23 mar 2012 - end
		
		//yong 23 mar 2012 - type graph
		//include type graph from queries,productions,rules
		Object rggGraphObj = registry.getProjectGraph().getProperty (RGGGraph.XL_GRAPH);
		if(rggGraphObj!=null)
		{
			((RGGGraph)rggGraphObj).setDoNotEnumerateTypeGraph(false);
		}
		//yong 23 mar 2012 - end
		...
		//yong 23 mar 2012 - type graph
		//exclude type graph from queries,productions,rules
		if(rggGraphObj!=null)
		{
			((RGGGraph)rggGraphObj).setDoNotEnumerateTypeGraph(true);
		}
		//yong 23 mar 2012 - end
	}
