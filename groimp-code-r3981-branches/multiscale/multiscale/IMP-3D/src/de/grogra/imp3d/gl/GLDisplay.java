		...
		RenderState, ImageObserver, Selectable
{
	...
	private static final boolean USE_GLJPANEL = 
...
//				float attenuationExponent = spotLight.getAttenuationExponent ();
	...
	public void init (GLAutoDrawable d)
	{
		...
		// allocate text renderer to provide accelerated text rendering that works with opengl
//		textRenderer = new TextRenderer(new Font("SansSerif", Font.BOLD, 36));
	...
	/**
	 *  Generate the opengl commands to generate the tetrahedron data.
	 */
	void drawTetrahedronRegImpl(GL gl, float length)
	{
		float baseExtendLength = (length/2.0f) / (float)(Math.sin(Math.PI/3.0));
		float perpendicularHeight = (float) Math.sqrt((length*length)-(baseExtendLength*baseExtendLength));
		
		Vector3f base1 = new Vector3f(
				(float)(baseExtendLength*Math.cos(0)),
				(float)(baseExtendLength*Math.sin(0)),
				-perpendicularHeight
				);
		Vector3f base2 = new Vector3f(
				(float)(baseExtendLength*Math.cos(2.0*Math.PI/3.0)),
				(float)(baseExtendLength*Math.sin(2.0*Math.PI/3.0)),
				-perpendicularHeight
				);
		Vector3f base3 = new Vector3f(
				(float)(baseExtendLength*Math.cos(2.0*Math.PI/3.0*2.0)),
				(float)(baseExtendLength*Math.sin(2.0*Math.PI/3.0*2.0)),
				-perpendicularHeight);
		
		gl.glBegin (GL.GL_TRIANGLE_FAN);
		gl.glNormal3f(0, 0, 1);
		gl.glVertex3f (0,0,0);
		gl.glNormal3f(base1.x,base1.y,0);
		gl.glVertex3f (base1.x,base1.y,base1.z);
		gl.glNormal3f(base2.x,base2.y,0);
		gl.glVertex3f (base2.x,base2.y,base2.z);
		gl.glNormal3f(base3.x,base3.y,0);
		gl.glVertex3f (base3.x,base3.y,base3.z);
		gl.glNormal3f(base1.x,base1.y,0);
		gl.glVertex3f (base1.x,base1.y,base1.z);
		gl.glEnd();
		
		gl.glBegin(GL.GL_TRIANGLES);
		gl.glNormal3f(base1.x,base1.y,base1.z);
		gl.glVertex3f (base1.x,base1.y,base1.z);
		gl.glNormal3f (base2.x,base2.y,base2.z);
		gl.glVertex3f (base2.x,base2.y,base2.z);
		gl.glNormal3f(base3.x,base3.y,base3.z);
		gl.glVertex3f (base3.x,base3.y,base3.z);
		gl.glEnd();
	}
	...
	public void drawTetrahedronReg(float length, Shader s, int highlight, Matrix4d t)
	{
		...
		// call draw 
		drawTetrahedronRegImpl(gl,length);
