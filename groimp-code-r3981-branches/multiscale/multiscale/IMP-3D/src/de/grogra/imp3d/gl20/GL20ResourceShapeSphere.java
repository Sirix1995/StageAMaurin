...
public class GL20ResourceShapeSphere extends GL20ResourceShape {
	 ...
	 */
	final private static String MESH_SPHERE_NAME = new String("SPHERE");

	/**
	 * radius attribute bit
	 */
	final private static int RADIUS = 0x1;
	 ...
	 */
	final private PolygonArray createSpherePolygonArray(int subDivisions) {
		if (subDivisions < 3)
			return null;

		PolygonArray polygonArray = new PolygonArray();
		polygonArray.init(3);		// 3 components per vertex (x,y and z)
		...
		for (int vStep=0;vStep <= SUB_DIVS;vStep++) {
			if (vStep == 0) {
				...
				for (int hStep = 0;hStep <= SUB_DIVS;hStep++) {
					polygonList.add(vertexCount - 1); polygonList.add(vertexCount - SUB_DIVS - 1 + hStep); polygonList.add(vertexCount - SUB_DIVS - 2 + hStep);
				}
			}
			else {
				...
				if (vStep == 1)  {
					// first row of triangles below northpole
					for (int hStep=0;hStep <= SUB_DIVS;hStep++) {
						...
						uvList.add(oneOverSUB_DIVS * (float)hStep); uvList.add(texCoordV);
						vertexList.add(x); vertexList.add(y); vertexList.add(height);
					...
					for (int hStep=0;hStep <= SUB_DIVS;hStep++) {
						...
						if (hStep > 0) {
							polygonList.add(vertexCount - 2); polygonList.add(vertexCount - 1); polygonList.add(vertexCount - SUB_DIVS - 3);
							polygonList.add(vertexCount - SUB_DIVS - 3); polygonList.add(vertexCount - 1); polygonList.add(vertexCount - SUB_DIVS - 2);
						}
					}
				}
			}
	...
	public GL20ResourceShapeSphere() {
		...
		if (mesh == null) {
			// no mesh with the MESH_SPHERE_NAME was found, so create one
	 ...
	 */
	final public void setRadius(float radius) {
