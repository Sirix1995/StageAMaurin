		...
		Selectable
{
	...
	private Layer[] layers, layerStack = new Layer[10];
	...
	public void drawTetrahedronReg(float length, Shader sh, int highlight, Matrix4d t)
	{
		if (sh == null)
		{
			sh = visitor.getCurrentShader ();
...
//				t);
		...
		setVertex(2,
				baseExtendLength*Math.cos(2.0*Math.PI/3.0),
				baseExtendLength*Math.sin(2.0*Math.PI/3.0),
				-perpendicularHeight,
				t);
		setVertex(3,
				baseExtendLength*Math.cos(2.0*Math.PI/3.0*2.0),
				baseExtendLength*Math.sin(2.0*Math.PI/3.0*2.0),
				-perpendicularHeight,
				t);
		
		//draw lines
		drawLine(0,1,selected);
		drawLine(0,2,selected);
		drawLine(0,3,selected);
		drawLine(1,2,selected);
		drawLine(2,3,selected);
		drawLine(3,1,selected);
	...
	public void drawSupershape (float a, float b, float m1, float n11, float n12, float n13, float m2, float n21, float n22, float n23, Shader sh, int highlight, Matrix4d t)
	{
		...
		{
			theta = (float) (-M_PI + M_2PI * (float) u / (float) uvCount);
			// for each vertical slice
			for (int v = 0; v <= uvCount; v++)
			{
				// calculate next vertex
				phi = (float) (-M_PI_2 + M_PI * (float) v / (float) uvCount);
