...
//int lambdaMin			// D65: 300	// RGB: 390	// XYZ: 360
//int lambdaMax
//int lambdaSteps

//int colorMatchTexSize 	= 39;					// D65:	42	// RGB: 18	// XYZ: 39
uniform float S0xyzX;
uniform float S0xyzY;
uniform float S0xyzZ;

uniform float S1xyzX;
uniform float S1xyzY;
uniform float S1xyzZ;

uniform float S2xyzX;
uniform float S2xyzY;
uniform float S2xyzZ;
...
vec3 getXYZMatch(int lambda)
{
	...
	int minLMatch	= 360; // Don't change: This is hardcoded!

	int pos 		= int((lambda - minLMatch) / float(lambdaSteps));
...
float bbTemp = 6500;                 /* Hidden temperature argument
                                         to BB_SPECTRUM. */
...
/*                          CONSTRAIN_RGB
    ...
    to make RGB all positive.  The function returns true if the
    components were modified, false otherwise.
...
vec3 convert2RGB(vec3 xyz)
{
	...
	/*
	 * Possible Colour systems:	NTSCsystem, EBUsystem, SMPTEsystem,
	 * 							HDTVsystem, CIEsystem, Rec709system
	 */
	colourSystem cs	= EBUsystem;

	//xyz *=  xyz.y / (683.002f);

	// With these xyz values compute the conversion to RGB color space.
	rgb 			= xyz_to_rgb(cs, xyz);

	// Ensure that the values lies in a propper domain.
	constrain_rgb(rgb);

	//norm_rgb(rgb);
	//gamma_correct_rgb(cs, rgb);

	// Apply the brightness to a range which can displayed by monitors.
