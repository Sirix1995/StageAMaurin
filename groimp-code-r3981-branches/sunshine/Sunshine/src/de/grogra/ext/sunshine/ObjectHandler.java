...
{
	// 1 value for r * 4 byte: size of a float variable	
	public static final int R 			= BufferUtil.SIZEOF_FLOAT;
	// 2 values for rgb * 4 byte: size of a float variable	
	public static final int RG 			= 2 * BufferUtil.SIZEOF_FLOAT;
	// 3 values for rgb * 4 byte: size of a float variable	
	...
	private static int SPHERE_PIXEL 	= 7;
	private static int BOX_PIXEL 		= 7;
	private static int PLANE_PIXEL 		= 7;
	private static int PARA_PIXEL 		= 8;
	private static int CFC_PIXEL 		= 8;
	private static int TRIANGLE_PIXEL 	= 5;
	...
	public ObjectHandler(int sceneSize, int[] objects, int[] triangles, Hashtable<Image, Integer> imagCache)
	{
		...
		boxPart 	= 				objects[SPHERE]	* SIZEOF_SPHERE;
		cfcPart		= boxPart 	+ 	objects[BOX]	* SIZEOF_BOX;
		meshPart	= cfcPart 	+ 	objects[CFC]	* SIZEOF_CFC;
		paraPart	= meshPart 	+ 	objects[4]		* SIZEOF_MESH + triPart;
		planePart	= paraPart 	+ 	objects[5]		* SIZEOF_PARA;
		lightPart	= planePart + 	objects[PLANE]	* SIZEOF_PLANE;
		...
		countObjects(objects, triangles);
	} //constructor
	...
	{		
		switch(sObject.getID())
		{
			...
			case TRI: 		storeTriangle(sObject); break;
	...
	public void setSpherePartAsBBSphereID()
	{
		bbSphereID = sphereCount*SIZEOF_SPHERE;
	}
	
	public int getBSphereID()
	{
		return bbSphereID >= 0 ? bbSphereID / RGBA : -1;
	}
	...
	public int getParaPart()
	{
		return paraPart / RGBA;
	}
	...
	private void storeTriangle(SunshineObject sob)
	{
		...
		hasNormals 			= ((SunshineTriangles) sob).hasNormals();
		...
		for(int i = 0; i < indexList.length; i++  )
		{
			...
			if(hasNormals)
			{
				...
				sceneTex.putFloat( 0.0f );	// x value of a normal
				sceneTex.putFloat( 0.0f );	// y value of a normal
				sceneTex.putFloat( 0.0f );	// z value of a normal
			}
	...
	private void countObjects(int[] objects, int[] triangles)
	{
		int triCount = 0;
		...
		for(int i = 0; i < objects.length; i++)
			objectCount += i != 4 ? objects[i] : triCount;
...
//			System.err.println("Nicht in Cache gefunden, kann gar nicht sein!");
	...
	/**
	 * Delegates the converts request to static implementation.
	 * 
	 * @return the intbuffer
	 */
	public IntBuffer getPixels(int pos)
	{		
		return getPixels(images[pos]);
	}
	 ...
	 * @return the intbuffer
	 */
	public static IntBuffer getPixels(Image img)
	{
		int k = 512;
		...
		catch (InterruptedException e)
		{
			...
			System.err.println("could not grab pixel");
		}
	...
	{
		return sphereCount * SPHERE_PIXEL;
	}

	public int getBoxID() 
	{
		return (boxPart / 16) + boxCount * BOX_PIXEL;
	}

	public int getPlaneID() 
	{
		return (planePart / 16) + planeCount * PLANE_PIXEL;
	}

	public int getParaID() 
	{
		return (paraPart / 16) + paraCount * PARA_PIXEL;
	}
	
	public int getCfcID() 
	{
		return (cfcPart / 16) + cfcCount * CFC_PIXEL;
	}
	
	public int getTriangleID()
	{
		return (triPart / 16) + triCount * TRIANGLE_PIXEL;
	}	
} //class
