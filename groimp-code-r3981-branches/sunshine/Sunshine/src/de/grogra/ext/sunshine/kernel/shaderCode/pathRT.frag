...
void setRandomRgbaRay(vec3 outRay, vec3 spectrum, vec3 normal, vec4 color,
		int id)
{
	float ior = getIOR(id);
	...
	else
	{
		...
		secRay.direction 	= normalize(diffBasis * dir);
		secRay.spectrum 	= spectrum * color.rgb;
		secRay.density 		= pd * cost * _PI;
	}
...
void setRandomPhongRay(vec3 outRay, vec3 spectrum, vec3 normal, Phong phong)
{
	vec3 rdir;
	vec3 tdir;
	...
	else
	{
		...
		secRay.direction = normalize(m * vec3(cos(phi)*sint, sin(phi)*sint, cost));
...
void getRandomRay(Ray inRay, vec3 normal, TraceData td)
{
	vec4 material = getObjectColor(td.type == TRI ? meshPart : td.id);

	if(material.x == -1.0) //phong
	{
		Phong p = getPhong(material.y, td);
		setRandomPhongRay(-inRay.direction, inRay.spectrum, normal, p);
	}
	else
	{
		setRandomRgbaRay(-inRay.direction, inRay.spectrum, normal, material,
				td.id);
	}
}

void setSecRayDirection(Ray ray, vec3 normal, TraceData td)
{
