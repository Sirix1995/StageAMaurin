...
void main(void)
{
	...
	float seedX		= texture2DRect(outputImage1, gl_FragCoord.xy).w;
	float seedY		= texture2DRect(outputImage2, gl_FragCoord.xy).w;

	random2D		= vec2(seedX, seedY);
	...
	//pixelColor = vec4(ray, 1.0);

//	pixelColor = vec4(333, 222, random2D);

	gl_FragData[0]	= vec4(pixelColor					); 	//outputImage0
	gl_FragData[1]	= vec4(rayOrigin, 		random2D.x	);	//outputImage1
	gl_FragData[2]	= vec4(normalize(ray),	random2D.y	);	//outputImage2
