...
public class TraceRayKernel extends Kernel
{
	...
	public TraceRayKernel(String name, GLAutoDrawable drawable, int tileWidth, 
			int tileHeight, int[] sceneTexture, int[] texTexture, int[] kdTexture,
			boolean img)
	{
...
//	@Override
			...
			int[] inputTexture)
	{
		...
		useProgram(drawable);
			...
			gl.glBindTexture(texTarget, inputTexture[0]);
			setUniformTex(a0Loc, 0, drawable);
			
			gl.glActiveTexture(GL_TEXTURE1);
			gl.glBindTexture(texTarget, inputTexture[1]);
			...
			gl.glBindTexture(texTarget, inputTexture[3]);
