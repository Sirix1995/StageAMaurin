...
public class Frustum extends ObjectBase {
	...
	void exportImpl (Leaf node, InnerNode transform, STLExport export) throws IOException {
		...
		pos.set (0, 0, 0);
		m.transform (pos);
		p.vertices.push ((float) pos.x).push ((float) pos.y).push (
			(float) pos.z);
		pos.set (0, 0, l);
		m.transform (pos);
		p.vertices.push ((float) pos.x).push ((float) pos.y).push (
		...
		for (int u = 0; u < uCount; u++)
		{
			...
			p.vertices.push ((float) p0.x).push ((float) p0.y).push (
				(float) p0.z);
			p.vertices.push ((float) p1.x).push ((float) p1.y).push (
				(float) p1.z);
			...
			// top face
			p.polygons.push (1).push (index + 1).push (
				index + 1 + 2 * (uCount - u > 1 ? 1 : 1 - uCount)).push (1);
			// mantle face
