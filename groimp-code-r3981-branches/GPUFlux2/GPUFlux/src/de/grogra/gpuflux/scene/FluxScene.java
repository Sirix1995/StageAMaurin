...
public class FluxScene {
	...
	public void buildSceneFromGraph(Graph graph, ViewConfig3D view, ObjectFilter measureFilter, boolean enableSensors, ProgressMonitor monitor, boolean sampleExplicit, float flatness)
	{
		...
		//defaultLights = sceneVisitor.getLights();
		...
		if( performance == FluxSettings.PERFORMANCE_TRACE )
			bvhBuilder = new ThreadedBVHBuilderSAH();
		else
			bvhBuilder = new ThreadedBVHBuilderMiddle();
	...
	public String getSceneStats() {
		...
		stats += "    <b>Scene</b>\n";
		if( primitives != null ) 
			stats += "    Number primitives: " + primitives.size() + "\n";
		if( infPrimitives != null ) 
			stats += "    Number inf prims:  " + infPrimitives.size() + "\n";
		if( sensors != null ) 
			stats += "    Number sensors:    " + sensors.size() + "\n";
		//if( defaultLights != null ) 
		//	stats += "    Number lights:     " + defaultLights.size() + "\n";
		if( channels != null ) 
			stats += "    Number channels:   " + channels.size() + "\n";
		if( shaders != null ) 
			stats += "    Number shaders:    " + shaders.size() + "\n";
	...
	/*
	public Vector<LightNode> getDefaultLights() {
