...
public class MeasuringSetup {
	...
	private final Vector<FluxDetector> detectors = new Vector<FluxDetector>();
	...
	public Vector<Measurement> LoadMeasuredData(float[][] data, double scale) {
        ...
        for( int i = 0 ; i < getNumMeasurement() ; i++) {
        	...
        	for( int k = 0 ; k < getDepth()-1; k++ )
        		for( int j = 0 ; j < getDimensions(); j++ )
            		measurement.data[k][j] = data[k][i*(getDimensions())+j];
	...
	public Measurement createMeasurement() {
		return new Measurement(getDepth(), getDimensions());
	}
