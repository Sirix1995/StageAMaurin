...
public class FluxSceneVisitor extends DisplayVisitor {
	...
	private final ObjectList<Interior> interiorStack = new ObjectList<Interior> ();
	
	private final Vector<FluxPrimitive> primitives = new Vector<FluxPrimitive>();
	private final Vector<FluxPrimitive> infPrimitives = new Vector<FluxPrimitive>();
	private final Vector<LightNode> lights = new Vector<LightNode>(); 
	private final Vector<FluxSensor> sensors = new Vector<FluxSensor>();
	...
	{
		return (shader instanceof AlgorithmSwitchShader) ? ((AlgorithmSwitchShader) shader)
			.getRaytracerShader ()
	...
	public void visitScene(Graph graph, ViewConfig3D view, ObjectFilter measureFilter, boolean enableSensors, boolean sampleExplicit, ProgressMonitor monitor, float flatness, boolean[] visibleLayers) {
		...
		// finish the construction all all gathered objects
		volumeBuilder.finish();
		lightBuilder.finnish( sampleExplicit, view );
		sceneBuildTime = System.currentTimeMillis () - sceneBuildTime;
		
		// compute the primitives bounding volumes
		Variables temp = new Variables();
		for(FluxPrimitive prim: primitives)
			prim.computeExtent(temp );
		for(FluxPrimitive prim: infPrimitives)
			prim.computeExtent(temp);
		for(FluxSensor prim: sensors)
			prim.computeExtent(temp);
	}
	...
	private void addPrimitives(Vector<FluxPrimitive> prim) {
		if( prim.size() > 0 )
		{
			...
			FluxShader fluxShader = shaderBuilder.buildShader( currentShader );
			for( int i = 0 ; i < prim.size() ; i++ )
			{
				prim.get(i).setGroupIndex( groupIndex );
				prim.get(i).setOwner( currentRenderable );
				prim.get(i).setFluxShader( fluxShader );
	...
	private void addPrimitive(FluxPrimitive prim) {
		...
		if ((primitives.size() % 50) == 0)
		{
			...
			if (t >= nextProgressTime)
			{
				...
				monitor.setProgress (IMP3D.I18N.msg ("ray.constructing-geometry",
						primitives.size()),
	...
	protected void visitImpl(Object object, boolean asNode, Shader s, Path path) {
		...
		Interior i = state.getObjectDefault (object, asNode,
				Attributes.INTERIOR, currentInterior);
		if (i != null)
		{
			currentInterior = i;
		}
		...
		if( currentInterior != null )
			currentIOR = currentInterior.getIndexOfRefraction(new Spectrum3d(( (rgb >> 16) & 255) / 255f, ((rgb >> 8) & 255) / 255f, (rgb & 255) / 255f ) );
		else
			currentIOR = 1.f;
		
		int csg = state.getIntDefault (object, asNode, Attributes.CSG_OPERATION,
				Attributes.CSG_NONE);
		
		if( csg != Attributes.CSG_NONE )
			warning( "GPUFlux does not support CSG" );
		...
		if (shape instanceof Light)
		{
			...
			if(shape instanceof Parallelogram) {
				//and iff a light is set 
				if(((Parallelogram)shape).getLight() != null) {
					// add it
					Matrix4d t = getCurrentTransformation ();
					lightBuilder.buildLight( (Light)object , t );
				}
				if(((Parallelogram)shape).getAreaLight() != null) {
					// add it
					Matrix4d t = getCurrentTransformation ();
					lightBuilder.buildLight( (Light)shape , t );
				}
			} else {
				Matrix4d t = getCurrentTransformation ();
				lightBuilder.buildLight( (Light)object , t );
		...
		if (shape instanceof Light)
		{
			...
			lightBuilder.buildLight( light , t );
		}*/
