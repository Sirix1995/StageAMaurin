...
public class FluxSettings {
	...
	public static final EnumerationType PERFORMANCE_TYPE = new EnumerationType (
		"performance", GPUFlux.I18N, PERFORMANCE_TRACE + 1);
	
	private static final int DEFAULT_MIN_MEASUREMENTS = 1024*8;
	private static final int DEFAULT_TRACE_DEPTH = 5;
	private static final int DEFAULT_TRACE_RANDOMSEED = 42;
	...
	{
		if( general == null )
			general = Item.resolveItem (Workbench.current (), "/renderers/3d/GPUFluxRenderer/general");
		if( ocl == null )
			ocl = Item.resolveItem (Workbench.current (), "/renderers/3d/GPUFluxRenderer/opencl");
		if( advanced == null )
			advanced = Item.resolveItem (Workbench.current (), "/renderers/3d/GPUFluxRenderer/opencl/advanced");
		if( renderer == null )
			renderer = Item.resolveItem (Workbench.current (), "/renderers/3d/GPUFluxRenderer/renderer");
	}
	...
	{
		//getItems();
		//return Utils.getInt (advanced, "rayprocessor.maximumsamplecount" , MAXIMUM_SAMPLE_COUNT);
	...
	{
		getItems();
		return Utils.getInt(renderer, "rayprocessor.randomseed",DEFAULT_TRACE_RANDOMSEED);
	}
