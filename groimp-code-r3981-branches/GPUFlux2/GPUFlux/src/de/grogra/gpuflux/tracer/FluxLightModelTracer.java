...
public class FluxLightModelTracer extends ShareableBase implements ProgressMonitor 
{
	...
	private transient Experiment absorbedPower;
	private transient Experiment reflectedPower;
	private transient Experiment transmittedPower;
	private transient Experiment sensedPower;
...
// NOTE: The following lines up to enh:end were generated automatically
	...
	public static class Type extends SCOType
	{
		...
		protected void setBoolean (Object o, int id, boolean value)
		{
			switch (id)
			{
				case Type.SUPER_FIELD_COUNT + 9:
					((FluxLightModelTracer) o).enableSensors = value;
					return;
				case Type.SUPER_FIELD_COUNT + 15:
					((FluxLightModelTracer) o).dispersion = value;
					return;
			}
		...
		protected void setInt (Object o, int id, int value)
		{
			switch (id)
			{
				case Type.SUPER_FIELD_COUNT + 6:
					((FluxLightModelTracer) o).depth = value;
					return;
				case Type.SUPER_FIELD_COUNT + 7:
					((FluxLightModelTracer) o).randomseed = value;
					return;
				case Type.SUPER_FIELD_COUNT + 8:
					((FluxLightModelTracer) o).rayCount = value;
					return;
				case Type.SUPER_FIELD_COUNT + 11:
					((FluxLightModelTracer) o).minLambda = value;
					return;
				case Type.SUPER_FIELD_COUNT + 12:
					((FluxLightModelTracer) o).spectralBuckets = value;
					return;
			}
		...
		protected int getInt (Object o, int id)
		{
			switch (id)
			{
				...
				case Type.SUPER_FIELD_COUNT + 7:
					return ((FluxLightModelTracer) o).getRandomseed ();
				case Type.SUPER_FIELD_COUNT + 8:
		...
		protected void setFloat (Object o, int id, float value)
		{
			switch (id)
			{
				case Type.SUPER_FIELD_COUNT + 2:
					((FluxLightModelTracer) o).flatness = value;
					return;
			}
		...
		protected void setDouble (Object o, int id, double value)
		{
			switch (id)
			{
				case Type.SUPER_FIELD_COUNT + 3:
					((FluxLightModelTracer) o).minPower = value;
					return;
				case Type.SUPER_FIELD_COUNT + 4:
					((FluxLightModelTracer) o).targetVariance = value;
					return;
				case Type.SUPER_FIELD_COUNT + 5:
					((FluxLightModelTracer) o).cutoffPower = value;
					return;
			}
	...
	static
	{
		...
		randomseed$FIELD = Type._addManagedField ($TYPE, "randomseed", Type.Field.PRIVATE  | Type.Field.SCO, de.grogra.reflect.Type.INT, null, Type.SUPER_FIELD_COUNT + 7);
		rayCount$FIELD = Type._addManagedField ($TYPE, "rayCount", Type.Field.PRIVATE  | Type.Field.SCO, de.grogra.reflect.Type.INT, null, Type.SUPER_FIELD_COUNT + 8);
		enableSensors$FIELD = Type._addManagedField ($TYPE, "enableSensors", Type.Field.PRIVATE  | Type.Field.SCO, de.grogra.reflect.Type.BOOLEAN, null, Type.SUPER_FIELD_COUNT + 9);
		measureMode$FIELD = Type._addManagedField ($TYPE, "measureMode", Type.Field.PRIVATE  | Type.Field.SCO, de.grogra.reflect.ClassAdapter.wrap (MeasureMode.class), null, Type.SUPER_FIELD_COUNT + 10);
		minLambda$FIELD = Type._addManagedField ($TYPE, "minLambda", Type.Field.PRIVATE  | Type.Field.SCO, de.grogra.reflect.Type.INT, null, Type.SUPER_FIELD_COUNT + 11);
		spectralBuckets$FIELD = Type._addManagedField ($TYPE, "spectralBuckets", Type.Field.PRIVATE  | Type.Field.SCO, de.grogra.reflect.Type.INT, null, Type.SUPER_FIELD_COUNT + 12);
		importanceCurve$FIELD = Type._addManagedField ($TYPE, "importanceCurve", Type.Field.PRIVATE  | Type.Field.SCO, de.grogra.reflect.ClassAdapter.wrap (SpectralCurve.class), null, Type.SUPER_FIELD_COUNT + 13);
		sensitivityCurves$FIELD = Type._addManagedField ($TYPE, "sensitivityCurves", Type.Field.PRIVATE  | Type.Field.SCO, de.grogra.reflect.ClassAdapter.wrap (SpectralCurve[].class), null, Type.SUPER_FIELD_COUNT + 14);
		dispersion$FIELD = Type._addManagedField ($TYPE, "dispersion", Type.Field.PRIVATE  | Type.Field.SCO, de.grogra.reflect.Type.BOOLEAN, null, Type.SUPER_FIELD_COUNT + 15);
		$TYPE.validate ();
	}
...
//enh:end
	...
	{
		// check if seed is invalid, if so throw an exception
		if (seed <= 0)
			throw new IllegalArgumentException (
				"seed must be a positive integer but was " + seed);
	...
	class DeviceMonitor extends Thread
	{
		...
		String log = "";
		Buffer powerAbsorbedBuffer, powerReflectedBuffer, powerTransmittedBuffer, powerSensedBuffer, irradianceBuffer;
	...
	public void compute (boolean forceCompute, boolean forceRebuild)
	{
		...
		try
		{
			...
			{
				// release last experiments
				irradiance = null;
				absorbedPower = null;
				reflectedPower = null;
				transmittedPower = null;
				sensedPower = null;
				...
				measuringSetup.setDepth(getDepth());
				...
				for(int i11 = 0 ; i11 < deviceList.size(); i11++ )
				{
					...
					monitor.powerAbsorbedBuffer = device.createBuffer(Sizeof.cl_float*measuringSetup.getDimensions()*measuringSetup.getNumMeasurement()*getDepth(), CL_MEM_READ_WRITE);
					monitor.powerAbsorbedBuffer.clear();

					monitor.irradianceBuffer = device.createBuffer(Sizeof.cl_float*measuringSetup.getDimensions()*measuringSetup.getNumMeasurement()*getDepth(), CL_MEM_READ_WRITE);
				...
				while( smpl < sampleCount )
				{
					...
					// set output buffers
					monitor.device.setKernelArgMemBuffer(kernel, 4, monitor.powerAbsorbedBuffer);
					...
					// set seed
					monitor.device.setKernelArgInt(kernel, 29, randomseed);
				...
				//absorbed
		        float[][] power = new float[measuringSetup.getDepth()][(measuringSetup.getDimensions())*measuringSetup.getNumMeasurement()];
		        for( DeviceMonitor monitor : deviceMonitors ) {
		        	int depthSliceSize = measuringSetup.getDimensions() * measuringSetup.getNumMeasurement();
		        
		        	byte powerData[] = new byte[Sizeof.cl_float*measuringSetup.getDepth()*depthSliceSize];
					monitor.powerAbsorbedBuffer.readBuffer(powerData);
					monitor.powerAbsorbedBuffer = null; // release memory
			        ...
			        for(int i=0; i<measuringSetup.getDepth(); i++) {
				        for(int i11 = 0; i11 < depthSliceSize; i11++ )
				        {
				        	power[i][i11] += powerBuffer.get(i*depthSliceSize+i11);
				        }
		        	}
		        }
		        this.absorbedPower = loadMeasuredData( power );
		        
		        /*
				//reflected
		        power = new float[measuringSetup.getDepth()][(measuringSetup.getDimensions())*measuringSetup.getNumMeasurement()];
		        for( DeviceMonitor monitor : deviceMonitors ) {
		        	int depthSliceSize = measuringSetup.getDimensions() * measuringSetup.getNumMeasurement();
		        	
		        	byte powerData[] = new byte[Sizeof.cl_float*measuringSetup.getDepth()*depthSliceSize];
					monitor.powerReflectedBuffer.readBuffer(powerData);
					monitor.powerReflectedBuffer = null; // release memory
		        ...
		        this.reflectedPower = loadMeasuredData( power );
		        //transmitted
		        power = new float[measuringSetup.getDepth()][(measuringSetup.getDimensions())*measuringSetup.getNumMeasurement()];
		        for( DeviceMonitor monitor : deviceMonitors ) {
		        	int depthSliceSize = measuringSetup.getDimensions() * measuringSetup.getNumMeasurement();
		        	...
		        	byte powerData[] = new byte[Sizeof.cl_float*measuringSetup.getDepth()*depthSliceSize];
					monitor.powerTransmittedBuffer.readBuffer(powerData);
					monitor.powerTransmittedBuffer = null; // release memory
		        ...
		        this.transmittedPower = loadMeasuredData( power );
				//sensed
		        power = new float[measuringSetup.getDepth()][(measuringSetup.getDimensions())*measuringSetup.getNumMeasurement()];
		        for( DeviceMonitor monitor : deviceMonitors ) {
		        	int depthSliceSize = measuringSetup.getDimensions() * measuringSetup.getNumMeasurement();
					...
					monitor.powerSensedBuffer.readBuffer(powerData);
					monitor.powerSensedBuffer = null; // release memory
		        ...
		        this.sensedPower = loadMeasuredData( power );
		        */
				power = null; // release memory
				
				//irradiance
		        float irradiance[][] = new float[measuringSetup.getDepth()][(measuringSetup.getDimensions())*measuringSetup.getNumMeasurement()];
		        for( DeviceMonitor monitor : deviceMonitors ) {
		        	int depthSliceSize = measuringSetup.getDimensions() * measuringSetup.getNumMeasurement();
					byte irradianceData[] = new byte[Sizeof.cl_float*measuringSetup.getDepth()*depthSliceSize];
			        ...
			        for(int i=0; i<measuringSetup.getDepth(); i++) {
				        for(int i11 = 0; i11 < depthSliceSize; i11++ )
				        {
				        	irradiance[i][i11] += irradianceBuffer.get(i*depthSliceSize+i11);
				        }
		        	}
	...
	private Experiment loadMeasuredData(float[][] data) {
		...
		return new Experiment(scene.getNodeToGroup(),measuringSetup.LoadMeasuredData(data, invRayCount), new Measurement( getDepth(), getMeasureDimensions() ));
	}
	...
	public Spectrum3d getSensedIrradiance (Node node)
	{
		...
		return new Spectrum3d(  m.integrateBucket(0), m.integrateBucket(1), m.integrateBucket(2) );
	}
	...
	{
		if( absorbedPower == null ) return new Measurement();
		return absorbedPower.getMeasurement(node);
	}
	...
	{
		if( reflectedPower == null ) return new Measurement();
		return reflectedPower.getMeasurement(node);
	}
	...
	{
		if( transmittedPower == null ) return new Measurement();
		return transmittedPower.getMeasurement(node);
	}
	...
	{
		if( sensedPower == null ) return new Measurement();
		return sensedPower.getMeasurement(node);
	}
