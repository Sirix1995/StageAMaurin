...
/*
 * Vorläufige Kommentierung in Deutsch: Klasse soll Dateien im DTD-Format einlesen und für GroIMP aufbereiten.
 * Ursprünglicher Autor: Dexu Zhao
 * Änderungen von: Jan Dérer
 ...
 * 						-> BUG1: Ursprünglich wurde eine Vektordifferenz verwendet, welche eine Vektoraddition war (wurde gefixt)
 * 						-> BUG2: Farbindex wurde eingelesen aber nicht beachtet (wurde gefixt)
 * 						-> Umbenennung von Attributen (Refactoring)
 * 						-> Vorläufige Entnahme vom Befehl T, welche bis dato dasselbe bewirkte wie J
 * 						-> BUG3: N-Parameter wurde nicht richtig an die Struktur übergeben (wurde gefixt)
 * 			08. 06. 07 	-> Kommentierung
 * 						-> Einige Methoden auf private gesetzt
 * 						-> Vorläufige Entnahme vom Befehl M, X und P
 ...
 * 						-> n-Parameter wurde als lokaler Turtle-Befehl eingefügt und ist auswertbar
 * 						-> heartwoodDiameter entfernt, da es nicht benötigt wird
 ...
 * 						-> Entfernen des NULL-Transformationsknoten, da F einen eigenen Transformationsknoten enthält
 * 						-> T, M, X, P werden gelesen aber nicht interpretiert
 * 						-> Verwendung des DTDShoot anstelle des F (DTDShoot leitet von F ab)
 * 			31. 07. 07	-> Wertübernahme von der generativen Distanz, Ordnung und relative Position auf dem Mutterspross
 * 						-> Verarbeitung von Blättern (B) und deren Positionierung mit \phyllotaxy
 * 						-> Blätter werden noch als rote Zylinder dargestellt
 * 						-> Verarbeitung des \leafarea-Befehls
 * 						-> Vorübergehend wird \leaflength und \leafbreadth für die Länge und den Durchmesser für den Zylinder verwendet
 * 						-> Verarbeitung von Früchten (F) und deren Positionierung
 * 						-> Früchte werden noch als blaue Zylinder dargestellt
 * 						-> Kommentierung
 ...
 * 						-> Unterstützung von mehreren root-Objekten in DTD-Dateien
 * 			10. 06. 09	-> BUG9: Leerzeilen führten zur Fehlermeldung (bedingt durch den Workaround für BUG8)
 */
public class DTDFilter extends FilterBase implements ObjectSource {
	...
	private static final float FACTOR = 0.01f;						// Faktor zum Umrechnen in Millimeter 
	private static final float DEFAULT_SHORT_SHOOT_LENGTH = 7.07f;	// Standardwert fï¿½r die Lï¿½nge eines Kurztriebs
	private static final int DEFAULT_DIRECTION = -5;				// Standardwert fï¿½r die Richtung
	private static final float DEFAULT_THICKNESS = 1.001f;			// Standardwert fï¿½r die Sprossdicke
	
	private static int leafCounter = 0;								// Zï¿½hlt die Gessamtzahl an Blï¿½ttern, um einen Namen zu vergeben fï¿½r den Shoot
	private static int fruitCounter = 0;							// Zï¿½hlt die Gessamtzahl an Frï¿½chten, um einen Namen zu vergeben fï¿½r den Shoot
	...
	private static final class DTDTokenizer extends Tokenizer {
	...
	public Object getObject() throws IOException {
		...
		try {
			...
			// Verändern der Datei so, dass vor jedem Bezeichner ein # steht
			...
			while (!in.isClosed()) {
				...
				if (token.getType() == DTDTokenizer.LAUBPARA) {
					...
					// Wurde ein Kopf (Name, Lï¿½nge/Kurztrieb und Muttername) gefunden und die Zeile/Datei ist zu Ende,
					// dann fï¿½hre schon jetzt die Sprossanalyse durch und fï¿½ge den Spross hinzu
			...
			// Knoten, welcher in den Graph eingefï¿½gt wird
			...
			while (enu.hasNext()) {
				...
				if (parent == null) {
					...
					// Wegen Unterstützung von mehreren root-Objekten auskommentiert
...
//					}
				...
				if(CLASSIC_VERSION) {
					...
					// Wurde ein n-Parameter gesetzt? -> Dann erzeuge einen Nl-Turtlebefehl fï¿½r die Wertï¿½bernahme (lokale Auswirkung)
					if(child.nad != 0.0f) {
						Nl nl = new Nl(child.nad);
						nl.addEdgeBitsTo(connector, Graph.SUCCESSOR_EDGE, null);
						connector = nl;
					}
					// Wurde die Ordnung gesetzt? -> Dann erzeuge einen OR-Turtlebefehl fï¿½r die Wertï¿½bernahme
					if(child.or != -1) {
						OR or = new OR(child.or);
						or.addEdgeBitsTo(connector, Graph.SUCCESSOR_EDGE, null);
						connector = or;
					}
					// Wurde die generative Distanz gesetzt? -> Dann erzeuge einen GD-Turtlebefehl fï¿½r die Wertï¿½bernahme
					if(child.gen != -1) {
						GD gd = new GD(child.gen);
						gd.addEdgeBitsTo(connector, Graph.SUCCESSOR_EDGE, null);
						connector = gd;
					}
					// Wurde die Internodienzahl gesetzt? -> Dann erzeuge einen Ul-Turtlebefehl fï¿½r die Wertï¿½bernahme
					if(child.izahl != 0) {
						Ul ul = new Ul(child.izahl);
						ul.addEdgeBitsTo(connector, Graph.SUCCESSOR_EDGE, null);
						connector = ul;
					}
					// Wurde der q-Wert gesetzt? -> Dann erzeuge einen MRel-Turtlebefehl fï¿½r die Wertï¿½bernahme
					// und setze beim Mutterspross einen L-Turtlebefehl ein (MRel bezieht sich auf die Lï¿½nge, welche mit L vergeben wird)
					if(child.q != 0.0f) {
...
//					}
			...
			// Herstellen der ursprünglichen DTD-Datei
...
//			Class classToInstanciated = Class.forName(filename.substring(0, filename.length()-4));
			...
			// LeafObject in den ObjectExplorer einhängen
	...
	private void loadRGGFileToProject() {
		...
		// Ist die RGG-Datei nicht im Projekt, dann füge sie ein
	...
	private void processingShoot(boolean vertif, ShootInfo motherShoot, HashMap<String, ShootInfo> shoots) {
		...
		if(shootinfo.akurztr > 0) {
			String basisName = shootinfo.name;		// Zwischenspeichern des Basisnamens fï¿½r die Gliednamen 
			...
			Vector3f beginShoot = (Vector3f) shootinfo.pend.clone();	// Anfangspunkt fï¿½r das nï¿½chste Glied, ist der Endpunkt des vorherigen Glieds
			...
			for(int i = 2; i <= shootinfo.akurztr; i++) {
				...
				clone.scale(FACTOR);
				endShoot.scale(clone.shoot.length, beginShoot);	// Neuen Endpunkt berechnet durch Anfangspunkt * Vektorlï¿½nge
		...
		// Liegen Blï¿½tter vor (und dafï¿½r entsprechende Einfï¿½gepositionen)?
		if(internodiumCount > 0 && leafCount > 0) {
			analyseLeaf(vertif, shoots, shootinfo);
		}
		
		// Liegen Frï¿½chte vor (und dafï¿½r entsprechende Einfï¿½gepositionen)?
		if(internodiumCount > 0 && fruitCount > 0) {
			analyseFruit(vertif, shoots, shootinfo);
		}
	}

	// Verarbeitung und Positionierung von Frï¿½chten
	private void analyseFruit(boolean vertif, HashMap<String, ShootInfo> shoots, ShootInfo shootinfo) {
		float bwink = 50.0f;	// Temporï¿½re Variable, um die Frï¿½chte rossettenartig am Sprossanfang zu Positionieren
								// (wenn keine Einfï¿½geposition mehr vorhanden ist)
		azimuthAngle = 90.0f;	// Initialisierung der Positionierung
		
		// Initialisierung der Parameter fï¿½r ein Frucht
		diameter = DEFAULT_THICKNESS;
		nParameter = 0.0f;
		shortShootCount = 0;
		treeColor = 4;
		...
		// Durchlaufe so oft, wie Frï¿½chte angegeben wurde
		for(int i = 1; i <= fruitCount; i++) {
			...
			// Existiert noch eine "freie" Einfï¿½geposition, dann berechne dafï¿½r den q-Wert
			...
			// Verarbeite die Frucht und fï¿½ge es in die Map ein
			processingLeaf(vertif, shootinfo, shoots, i, true);
		}
	}

	// Verarbeitung und Positionierung von Blï¿½ttern
	private void analyseLeaf(boolean vertif, HashMap<String, ShootInfo> shoots, ShootInfo shootinfo) {
		float bwink = 70.0f;	// Temporï¿½re Variable, um die Blï¿½tter rossettenartig am Sprossanfang zu Positionieren
								// (wenn keine Einfï¿½geposition mehr vorhanden ist)
		azimuthAngle = 90.0f;	// Initialisierung der Positionierung
		int oppositeCounter = 0;// Temporï¿½re Variable fï¿½r die Positionierung bei der Phyllotaxe Option "opposite"
		
		// Initialisierung der Parameter fï¿½r ein Blatt
		...
		// Durchlaufe so oft, wie Blï¿½tter angegeben wurde
		for(int i = 0; i < leafCount; i++) {
			...
			// Verarbeite das Blatt und fï¿½ge es in die Map ein
			processingLeaf(vertif, shootinfo, shoots, i, false);
		}
	}

	// Analog wie processingShoot nur ohne Kurztriebkette oder Blï¿½tter/Frï¿½chte
	private void processingLeaf(boolean vertif, ShootInfo motherShoot, HashMap<String, ShootInfo> shoots, int leafNr, boolean fruit) {
		...
		// Blï¿½tter/Frï¿½chte haben keine Internodien, zur Erkennung eines Blatt wird Anzahl auf -1 gesetzt und fï¿½r Frï¿½chte auf -2
		if(fruit)
			shootinfo.izahl = -2;
		else
			shootinfo.izahl = -1;
	...
	private void createBackUpFile(String dtdFile) {
		...
		try {
			...
			// Original Datei zum Lesen und Backup-Datei zum Schreiben öffnen
			...
			try {
				// Streams schließen (sofern geöffnet)
				if(in != null)
					in.close();
				if(out != null)
					out.close();
	...
	// Löscht die dtd-Datei und ändert den Namen der Backup-Datei in den ursprünglichen Namen um
	private void restoreBackUpFile(String dtdFile) {
		File manipulatedFile = new File(dtdFile);
		File backupFile = new File(dtdFile + ".tmp");
		
		// Wenn die manipulierte Datei existiert, dann Lösche diese
		// und ändern den Namen der Backup-Datei um
		if(manipulatedFile.exists() && backupFile.exists()) {
			manipulatedFile.delete();
			backupFile.renameTo(manipulatedFile);
		// Wenn nur die Backup-Datei existert, dann nur den Namen ändern
		} else if(!manipulatedFile.exists() && backupFile.exists()) {
			backupFile.renameTo(manipulatedFile);
		}
	}
	
	// Setzt in jede Zeile die mit einem Bezeichner für ein Ast-Stück anfängt
	...
	private void preparationOfTheDTDFile(String dtdFile) {
		...
		try {
			// Datei zum Lesen und zum Schreiben öffnen
