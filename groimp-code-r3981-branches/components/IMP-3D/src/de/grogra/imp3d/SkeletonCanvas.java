...
public class SkeletonCanvas extends CanvasAdapter implements RenderState,
		Selectable
{
...
//  optionAnaglyphMode
	...
	private class WFVisitor extends DisplayVisitor
	{
				...
				GraphState state)
		{
			...
			if ((c != null) && (c != this))
			{
				SkeletonCanvas.this.setColor ((Color3f) c, selectionState,
					false);
			}
			else
			{
				SkeletonCanvas.this.setColor (s.getAverageColor (),
	...
	private SkeletonCanvas (boolean justComputeMinMax)
	{
	...
	{
		SkeletonCanvas c = new SkeletonCanvas (true);
		c.initFactory (Item.resolveItem (view.getWorkbench (),
			View3D.DISPLAY_PATH + "/skeleton"));
...
//        optionAnaglyphMode                    
	...
	{
		getView3D ().getCanvasCamera ().setDimension (width, height);
	...
	@Override
	protected void paintScene (int flags, Graphics2D g) throws InterruptedException {
		if(mainIamge!=null) {
			mainIamge=null;
			return;
		}
		...
		if(optionStereoView || optionAnaglyphView) {
			...
			paintSceneBoby(flags, (Graphics2D)leftEyeImage.getGraphics (), false);
			...
			paintSceneBoby(flags, (Graphics2D)rightEyeImage.getGraphics (), false);
...
//			}
			...
			if(preview1!=0) {
				...
				paintSceneBoby(flags, (Graphics2D)gx2, false);
	...
	private void paintSceneBoby (int flags, Graphics2D g, boolean showGimmicks) throws InterruptedException {
		computeMinMax = true;
...
/*
{
	...
	IMP.writeImage(r.computeImage(), new File("DefaultView"+step+"_"+randomID+".exr"));
	...
	r.setRandomSeed(randomID);
	IMP.writeImage(r.computeImage(), new File("BirdView"+step+"_"+randomID+".exr"));
...
*/
			...
			int highlight, Matrix4d t)
	{
		...
		final Point3f origin = new Point3f(locations[0], locations[4], locations[8]);
		final Point3f end = new Point3f(locations[N-3], locations[N-2], locations[N-1]);
		drawLine(origin, end, color, highlight, null);
	}
	...
	@Override
	public void drawPlane (Shader sh, int highlight, Matrix4d t)
	{
		if (sh == null)
		{
	...
	public void drawBox (float halfWidth, float halfLength, float height,
			Shader sh, int highlight, Matrix4d t)
	{
		if (sh == null)
		{
			sh = visitor.getCurrentShader ();
	...
	@Override
	public void drawSphere (float radius, Shader sh, int highlight, Matrix4d t)
	{
		if (sh == null)
		{
			sh = visitor.getCurrentShader ();
		...
		setVertex (0, (float) (p0.x - v0.x), (float) (p0.y - v0.y), (float) (p0.z - v0.z));
		setVertex (1, (float) (p0.x + v0.x), (float) (p0.y + v0.y), (float) (p0.z + v0.z));
		drawLine (0, 1, (highlight & ViewSelection.SELECTED) != 0);
	}
	...
	public void drawSupershape (float a, float b, float m1, float n11, float n12, float n13, float m2, float n21, float n22, float n23, Shader sh, int highlight, Matrix4d t)
	{
		...
		genVertexSupershape (v1, 0, -M_PI_2, a, b, m1, n11, n12, n13, m2, n21, n22, n23);
		p.set(v1);
		t.transform(p);
		setVertex (0, p);
		genVertexSupershape (v1, 0, M_PI_2, a, b, m1, n11, n12, n13, m2, n21, n22, n23);
			...
			int highlight, Matrix4d t)
	{
		...
		setVertex (0, (float)p0.x,(float)p0.y,(float)p0.z);
		setVertex (1, (float)v0.x,(float)v0.y,(float)v0.z);
		drawLine (0, 1, (highlight & ViewSelection.SELECTED) != 0);
	}

	@Override
	public void drawParallelogram (float height, Vector3f secondAxis,
			float scaleU, float scaleV, Shader sh, int highlight, Matrix4d t)
	{
		if (sh == null)
		{
			sh = visitor.getCurrentShader ();
	...
	@Override
	public void drawPolygons (Polygonizable pz, Object obj, boolean asNode, Shader sh, int highlight, Matrix4d t)
	{
