...
public class RGG extends Node implements Selectable, RegistryContext
{
	...
	protected void startup ()
	{
		...
		registry.setMainRGG (MAIN_RGG_ID);
	...
	/**
	 * This method can will be invoked by <code>RGGR.reset()</code> 
	 * to reset the RGG project graph to its initial state. 
	 ...
	 * newly compiled RGG is instantiated.
	 */
	private void resetProjectGraph () {
	...
	/**
	 * This method can will be invoked by <code>RGGR.reset()</code> 
	 * to reset the RGG component graph to its initial state. 
	 * The following steps are performed:
	 * <ol>
	 * <li>
	 * If there is an instance of {@link ComponentRoot}
	 * which is connected with the real root of the
	 * graph by a branch edge, then this instance is removed.
	 * A new instance of <code>ComponentRoot</code>
	 * is created and connected with the real root
	 * by a branch edge.
	 * <li>
	 * A new instance of {@link AxiomComponent} is created
	 * and inserted in the graph by a successor edge
	 * from the <code>ComponentRoot</code> instance.
	 * The result of this step
	 * is a graph containing an <code>ComponentRoot</code>
	 * which has a single <code>AxiomComponent</code> as its
	 * successor.
	 * <li>
	 * The method {@link #init()} is invoked. This invocation
	 * is surrounded by transformation boundaries
	 ...
	 * newly compiled RGG is instantiated.
	 */
	private void resetComponentGraph () {
		Node root = (Node)registry.getProjectGraph ().getRoot (GraphManager.COMPONENT_GRAPH);
		Transaction xa = getPersistenceManager ().getActiveTransaction ();
		...
		// add a new root
		ComponentRoot rr = new ComponentRoot ();
		root.addEdgeBitsTo (rr, Graph.BRANCH_EDGE, null);
		rr.addEdgeBitsTo (new AxiomComponent (), Graph.SUCCESSOR_EDGE, null);
		...
		/*
		Node root2 = (Node)registry.getComponentGraph ().getRoot (GraphManager.COMPONENT_GRAPH);
		for (Edge e = root2.getFirstEdge (), f; e != null; e = f)
		{
			f = e.getNext (root2);
			if (e.testEdgeBits (Graph.BRANCH_EDGE)
				&& (e.getTarget () instanceof ComponentRoot))
			{
				e.remove (null);
			}
		}
		ComponentRoot rr2 = new ComponentRoot ();
		root2.addEdgeBitsTo (rr2, Graph.BRANCH_EDGE, null);
		rr2.addEdgeBitsTo (new AxiomComponent (), Graph.SUCCESSOR_EDGE, null);		
		
		System.out.println(" T1 = "+registry.getComponentGraph ().toXLString (false, "ComponentGraph"));
		System.out.println(" T2 = "+registry.getProjectGraph ().toXLString (false, "ComponentGraph"));
		*/
	}
