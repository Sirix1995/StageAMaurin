...
import de.grogra.pf.ui.util.componentdescription.GenerateDescription;
import de.grogra.pf.ui.util.componentdescription.LoadDescription;
		...
		TreeModelListener, Showable
{
		...
		if (m != null)
		{
			if (editable && m.isEditable ())
			{
				...
				for (int i = 0; i < p.length; i++)
				{
					...
					{
						t.closeDocument(getName (), ref); //close and clear buffer of document in JEditTextEditor before re-opening to refresh the view.
	...
	public void treeNodesChanged (TreeModelEvent e)
	{
		if (FileSystem.isContainedInChildren (getRegistry ().getProjectFile (
			getName ()), e))
		{
			// try to get a file called description.txt
			// if it has one i assume that it is a component
			if(IO.toFile (getRegistry (), "pfs:description.txt")!=null) {
				Object o = getRegistry ().getProjectFile ("pfs:description.txt");
				if(o!=null) {
					// and the description of this component needs to be updated
					GenerateDescription.generate(getRegistry ().getProjectFile (getName ()), o, getRegistry ().getFileSystem ());
	//				Object o1 = getRegistry ().getProjectFile (getName ());
			...
			refresh ((Item) getAxisParent (), this, deactivationCategory, new Command ()
				 {
					...
					public void run (Object info, Context context)
					{
						...
						//refresh description panel - yong 8 Mar 2012
						LoadDescription.loadPanel(context);
					}
				 });
...
// NOTE: The following lines up to enh:end were generated automatically
	...
	private static final class _Field extends NType.Field
	{
		...
		public void setBoolean (Object o, boolean value)
		{
			...
			{
				case 1:
					((SourceFile) o).editable = value;
					return;
				case 2:
					((SourceFile) o).disabled = value;
					return;
			}
