...
public final class ArrayPath implements Path
{
	...
	public void set (Path path)
	{
		...
		for (int i = count - 1; i >= 0; i--)
		{
			...
			if ((i & 1) != 0)
			{
				setEdges (i, path.getEdgeBits (i),
						  (byte) ((path.isInEdgeDirection (i) ? DIRECTION : 0)
								  + (path.isInstancingEdge (i) ? INSTANCING : 0)));
			}
		}
	}
	...
	public void pushNode (Object node, long id)
	{
		if ((count & 1) != 0)
		{
			throw new IllegalStateException
				("Path already ends in a node, cannot push another node.");
		}
		push (node, id);
	}
	...
	public void pushEdgeSet (Object edge, long id, boolean instancing)
	{
		...
		{
			throw new IllegalStateException
				("Path already ends in an edgeset, cannot push "
				 + "another edgeset.");
		}
	...
	public void pushEdges (int e, boolean dir, long id, boolean instancing)
	{
		...
		setEdges (count, e, (byte) ((dir ? DIRECTION : 0)
	...
	private void setEdges (int index, int e, byte info)
	{
		...
		else if (index >= (n = a.length))
		{
			System.arraycopy (a, 0,
							  edges = a = new int[2 * index + 8], 0,
							  n);
			System.arraycopy (d, 0,
							  edgeInfos = d = new byte[2 * index + 8], 0,
							  n);
		}
		d[index] = info;
		a[index] = e;
	...
	public Object popEdgeSet ()
	{
		...
		{
			throw new IllegalStateException
				("Path does not end in an edgeset.");
		}
		return pop ();
	}
	...
	public int getEdgeBits (int index)
	{
		...
		if ((index & 1) == 0)
		{
			throw new IllegalArgumentException
				("An index of an edgeset within a path has to be odd.");
		}
	...
	public boolean isInEdgeDirection (int index)
	{
		...
		Object e = objects[index];
		return (e != null)
			? graph.getSourceNode (e) == objects[index - 1]
			: (edgeInfos[index] & DIRECTION) != 0;
	}
	...
	public Object getNode (int index)
	{
		...
		{
			throw new IllegalArgumentException
				("An index of a node within a path has to be even.");
		}
	...
	public Object getEdge (int index)
	{
		...
		{
			throw new IllegalArgumentException
				("An index of a edge within a path has to be odd.");
		}
		return objects[index];
	}
