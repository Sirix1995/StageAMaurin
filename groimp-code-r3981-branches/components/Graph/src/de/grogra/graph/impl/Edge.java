...
public abstract class Edge implements Serializable
{
	...
	/**
	 * Checks the edgeBits of this edge against all possible edges
	 * and counts their occurrences. 
	 * 
	 * @return number of edges
	 */
	public final int getNumberOfEdge ()
	{
		int edgeCounter = 0;
		for (int i = Graph.MIN_NORMAL_BIT_INDEX; i < Graph.MAX_NORMAL_BIT_INDEX; i++) {
			final int mask = 1 << i;
			if (testEdgeBits(mask)) {
				edgeCounter ++;
			}
		}
		return edgeCounter;
...
//		if(s==null || t==null) return;
	...
	public static int parseEdgeKeys (String keys, Node source, Node target) throws SAXException
	{
		...
		while (i <= len)
		{
			...
			{
				if (keys.charAt (i)=='$' || keys.charAt (i)=='s')
				{
//System.out.println ("Bingo");
				...
				switch (keys.charAt (i))
				{
					...
					case 'B':
						...
						break;
					case 's':
						bits |= Graph.SEND_EDGE;
						break;
					case '~':
						bits |= Graph.USES_EDGE;
						break;
					case '$':
						bits |= Graph.SLOT_EDGE;
						break;
					case 'D':
						bits |= Graph.DUMMY_EDGE;
						break;
					case '0':
	...
	public void getEdgeKeys (StringBuffer out, boolean successorAsMinus, boolean useDescription)
	{
		...
		for (int i = Graph.MIN_NORMAL_BIT_INDEX; (b != 0) && (i < Graph.MAX_NORMAL_BIT_INDEX); i++)
		{
			int mask = 1 << i;
			if ((b & mask) != 0)
			{
	...
	public void getEdgeKeys (StringBuffer out)
	{
		int b = edgeBits;
		boolean comma = false;
		if ((b & Graph.SPECIAL_EDGE_MASK) != 0)
	...
	public boolean h_draw_flag = false;
	public boolean h_root_flag = false;
}
