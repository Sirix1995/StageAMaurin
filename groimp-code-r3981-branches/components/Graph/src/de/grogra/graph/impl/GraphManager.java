...
///////////////////////////////////////////////////////////////////////////////	
	...
	/**
	 * Returns the root node of the {@link Graph#COMPONENT_GRAPH} of this graph.
	 * 
	 * @return root node of component graph
	 */
	public Node getRootComponentGraph ()
	{
		return (Node) getRoot (COMPONENT_GRAPH);
	}	
		...
		if (commit && t.hasModified ())
		{
			...
			synchronized (makeLock)
			{
				...
				for (int i = hashBuckets.length - 1; i >= 0; i--)
				{
					...
					{
						if (((n.bits & Node.DELETED) == 0) && (n.getGCMark () != currentGCMark))
						{
	...
	private final ObjectList<Node> firstStack = new ObjectList<Node> (64);
	private final ObjectList<Node> secondStack = new ObjectList<Node> (64);
...
//!! *# End of generated code
	...
	@Override
	public final void accept (Object start, Visitor v, ArrayPath placeInPath, String graphType)
	{
		Node root = (Node) ((start != null) ? start : roots.get (graphType));
		if (root != null)
		{
	...
	public String getComponentGraphString () {
		String s = toXLString(false, COMPONENT_GRAPH, false);
		boolean hasAxiom = false;
		if(s.contains ("AxiomComponent")) hasAxiom = true;
		boolean hasWorld = false;
		if(s.contains ("World")) hasWorld = true;

		HashMap<Node, String> visited = new HashMap<Node, String> ();
		Node rootNode = (Node)getRoot (COMPONENT_GRAPH);
		// skip ComponentNode
		...
		StringBuffer buf = new StringBuffer ();
		append (buf, false, rootNode, null, false, visited, toVisit, false, false);
		s = buf.toString ();
		//clean if it was ComponentRoot AxiomComponent
		s = s.replace ("n0:ComponentRoot ", "");
		if(hasAxiom) {
			s = s.replace ("[> n1:AxiomComponent]", "World");
			s = s.replace ("[> AxiomComponent]", "World");
		}
		//clean if it was ComponentRoot ABC ..
		if(hasWorld) s = s.replace ("[> n1:World]", "World");
		...
		if (edge != null)
		{
			...
			switch (bits)
			{
				...
				case Graph.REFINEMENT_EDGE:
					buf.append (" /> ");
					break;
				case Graph.SEND_EDGE:
					buf.append ("-send-> "); //s>
					break;
				case Graph.SLOT_EDGE:
					buf.append (" -slot-> "); //$>
					break;
				case Graph.USES_EDGE:
					buf.append (" -uses-> ");//~>
					break;
				case Graph.DUMMY_EDGE:
					buf.append (" -dummy-> "); //D>
					break;
				case Graph.COMPONENT_INPUT_SLOT_EDGE:
					buf.append (" -ise-> ");
					break;
				case Graph.COMPONENT_OUTPUT_SLOT_EDGE:
					buf.append (" -ose-> ");
					break;
...
//					buf.append ("-0x").append (Integer.toHexString (bits)).append ("-> ");
					...
					if(edgeStyleText) {
						...
						tmp = tmp.replaceAll ("0x"+Integer.toHexString (Graph.COMPONENT_INPUT_SLOT_EDGE), "ise");
						tmp = tmp.replaceAll ("0x"+Integer.toHexString (Graph.COMPONENT_OUTPUT_SLOT_EDGE), "ose");
						tmp = tmp.replaceAll ("0x"+Integer.toHexString (Graph.DUMMY_EDGE), "dummy");
						buf.append (" -").append (tmp).append ("-> ");
					} else {
						edge.getEdgeKeys(edgeKeys);
						String tmp = edgeKeys.toString ();
						tmp = tmp.replaceAll ("0x"+Integer.toHexString (Graph.COMPONENT_INPUT_SLOT_EDGE), "ise");
						tmp = tmp.replaceAll ("0x"+Integer.toHexString (Graph.COMPONENT_OUTPUT_SLOT_EDGE), "ose");
						tmp = tmp.replaceAll (Integer.toString (Graph.COMPONENT_INPUT_SLOT_EDGE), "ise");
						tmp = tmp.replaceAll (Integer.toString (Graph.COMPONENT_OUTPUT_SLOT_EDGE), "ose");
						tmp = tmp.replaceAll ("0x"+Integer.toHexString (Graph.DUMMY_EDGE), "dummy");
						buf.append (" -(").append (tmp).append (")-> ");
					}
	...
	/**
	 * Number of nodes in the component graph.
	 * 
	 * @return 
	 */
	public int getComponentGraphSize() {
		return getGraphSize(COMPONENT_GRAPH);
	}
