...
public final class MimeType implements java.io.Serializable
{
	...
	/**
	 * The MIME type <code>text/sci</code> for greenscilab parameter files. 
	 */
	...
	{
		return (CLASS_PARAM.equals (name) && (clazz != null))
			? clazz.getName () : (String) params.get (name);
	}
	...
	{
		return mediaType.equals (TEXT_XML) || mediaType.equals (APPLICATION_XML)
	...
	{
		return isAssignableFrom (mediaType, s.mediaType)
			&& ((getParameter (WRAPPED_TYPE_PARAM) == null)
				|| isAssignableFrom (getParameter (WRAPPED_TYPE_PARAM),
									 s.getParameter (WRAPPED_TYPE_PARAM)))
			&& ((clazz == null)
				|| ((s.clazz != null) && clazz.isAssignableFrom (s.clazz)));
	}
	...
	public static boolean isAssignableFrom (String target, String source)
	{
		return (target.equals (source)
				|| target.equals (OCTET_STREAM.getMediaType ()))
			&& !(target.equals ("invalid/invalid")
				 || source.equals ("invalid/invalid"));
	}
	...
	{
		return (o == this)
			|| ((o instanceof MimeType)
				&& ((MimeType) o).mediaType.equals (mediaType)
				&& Utils.equal (((MimeType) o).clazz, clazz));
	}
	...
	public String toString ()
	{
		...
		for (int i = 0; i < p.size (); i++)
		{
			...
			if (v != null)
			{
				b.append ("; ").append (p.getKeyAt (i)).append ('=')
					.append (quoteParameter (v));
			}
		}
		if (clazz != null)
		{
			b.append ("; ").append (CLASS_PARAM).append ('=')
				.append (quoteParameter (clazz.getName ()));
		}
		if (comment != null)
		{
