		...
		if (e != null)
		{
			...
			if (updateWorkbenchSelection)
			{
				...
				else
				{
					...
					Object[] edges = new Object[a.length];
					...
					for (int i = a.length - 1; i >= 0; i--)
					{
						...
						states[i] = gs;
						// if it is an edge
						if( (((ArrayPath)a[i].getPath ()).getCount() & 1) == 0) {
							
							ArrayList list = new ArrayList(3);
							Object n1 = a[i].getPath ().getObject (l-1);
							Object n2 = a[i].getPath ().getObject (l);
							list.add (n1); // start node
							list.add (n2); // target node
							if(n1 instanceof Node && n2 instanceof Node) {
								list.add (((Node)n1).getEdgeBitsTo((Node)n2)); // edge type								
							} else if(n2 instanceof EdgeImpl) {
								list.add (-1);
							}
//							list.add (n1.getEdgeTo(n2)); // edge
						edges[i] = list;
						}
					...
					UIProperty.WORKBENCH_SELECTION.setValue
						(view, new GraphSelectionImpl (view, states, objects, edges, nodes));
				}
			}
		}
	}
