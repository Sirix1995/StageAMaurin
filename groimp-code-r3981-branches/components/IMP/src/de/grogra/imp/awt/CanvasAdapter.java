		...
		ImageObserver
{
	...
	public class CanvasComponent extends Component
	{
		...
		public void paint (Graphics g)
		{
			...
			// used to draw a line between the selected node and the tip of the mouse cursor 
			if(drawLineToMouseTip) {
				Graphics2D g2d = (Graphics2D)g;
				Stroke stroke = new BasicStroke(3);
				g2d.setStroke(stroke);
				g.setColor (new Color(10, 10, 175, 128));
				g.setColor (Color.blue);
				g.drawLine (x1, y1, x2, y2);
				g.fillOval (x1-3, y1-3, 6,6);
				stroke = new BasicStroke(1);
				g2d.setStroke(stroke);
				g.setColor (new Color(50, 250, 255, 128));
				g.drawLine (x1, y1, x2, y2);
			}
		}

		// true, iff a line between the selected node and the tip of the mouse cursor
		// should be drawn 
		private boolean drawLineToMouseTip = false;
		// coordinates of the line
		private int x1=0 , y1=0, x2=0 , y2=0;

		/**
		 * Sets the coordinates for the helper line between the selected node and the tip of the mouse.
		 * Currently only used by the ComponentView2D and invoked by the ComponentViewHierarchicalGraphModeEventHandler and 
		 * ComponentViewGraphModeEventHandler mouseMoved-function
		 * 
		 * @param x1
		 * @param y1
		 * @param x2
		 * @param y2
		 */
		public void setPointForLine(int x1, int y1, int x2, int y2) {
			this.x1 = x1;
			this.y1 = y1;
			this.x2 = x2;
			this.y2 = y2;
			drawLineToMouseTip = true;
			repaint();
		}

		/**
		 * has to be called to turn the line off again
		 */
		public void setLineOff() {
			drawLineToMouseTip = false;
			repaint();
		}
	}
