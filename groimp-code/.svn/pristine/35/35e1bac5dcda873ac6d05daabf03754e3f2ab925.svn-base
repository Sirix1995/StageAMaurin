...
public class XEGSphere {
	...
	public static void handleImportProperties(Node node, List<Property> properties, List<Property> handledProperties) {
		...
		for (Property p : properties) {
			if (p.getName().equals("radius")) {
				((Sphere) node).setRadius(Float.valueOf(p.getValue()));
	...
	public static void handleExportProperties(Node node, de.grogra.ext.exchangegraph.xmlbeans.Node xmlNode) {
		...
		Float radius = ((Sphere)node).getRadius();
		if (radius != null){
	...
	public static boolean isPropertiesValueSet(Node node){
		float propertyValue = ((Sphere) node).getRadius();
		Sphere defaultSphere = new Sphere();
		float defaultPropertyValue = defaultSphere.getRadius();
