...
public abstract class ObjectBase implements SceneGraphExport.NodeExport {
	...
	protected void writeVertices(Point4d[] p) {
		...
		for (int i = 0; i < p.length; i++) {
			outV.append(""+round(p[i].x)+" "+round(p[i].y)+" "+round(p[i].z)+" "+colString+"\n");
	...
	protected void writeFacets(String[] f) {
		...
		for (int i = 0; i < f.length; i++) {
			s = f[i].split(" ");
			outF.append("3");
			outF.append(" "+tmpIdx[Integer.parseInt(s[0])]);
			outF.append(" "+tmpIdx[Integer.parseInt(s[1])]);
			outF.append(" "+tmpIdx[Integer.parseInt(s[2])]+"\n");
		}
	...
	abstract void exportImpl (Leaf node, InnerNode transform, PLYExport export) throws IOException;
