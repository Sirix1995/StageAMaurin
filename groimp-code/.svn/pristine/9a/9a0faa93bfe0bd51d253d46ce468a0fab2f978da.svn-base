 ...
 * 
 */
public class Tetrahedron implements Iterable<OrientedFace>, Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = 4224553483608490276L;
	...
	private class FaceADB extends OrientedFace {
		...
		@Override
		public VoronoiCell getIncidentVertex() {
			return c;
		}

		@Override
		public VoronoiCell getVertex(int v) {
			switch (v) {
			case 0:
				return a;
			case 1:
		...
		@Override
		public boolean includes(VoronoiCell v) {
			if (a == v) {
				return true;
			}
			if (d == v) {
		...
		@Override
		public int indexOf(VoronoiCell v) {
			if (v == a) {
				return 0;
			}
			if (v == d) {
		...
		public boolean isConvex(int vertex) {
			VoronoiCell adjacentVertex = getAdjacentVertex();
		...
		@Override
		public int orientationOf(VoronoiCell query) {
		...
		@Override
		public VoronoiCell[] getEdge(VoronoiCell v) {
			switch (ordinalOf(v)) {
			case A: {
				return new VoronoiCell[] { d, b };
			}
			case D: {
				return new VoronoiCell[] { b, a };
			}
			case B: {
				return new VoronoiCell[] { a, d };
			}
			default:
	...
	private class FaceBCA extends OrientedFace {
		...
		public VoronoiCell[] getEdge(VoronoiCell v) {
			switch (ordinalOf(v)) {
			case B: {
				return new VoronoiCell[] { c, a };
			}
			case C: {
				return new VoronoiCell[] { a, b };
			}
			case A: {
				return new VoronoiCell[] { b, c };
			}
			default:
				throw new IllegalArgumentException("Invalid vertex ordinal");
			}
	...
	private class FaceCBD extends OrientedFace {
		...
		public VoronoiCell[] getEdge(VoronoiCell v) {
			switch (ordinalOf(v)) {
			case C: {
				return new VoronoiCell[] { b, d };
			}
			case B: {
				return new VoronoiCell[] { d, c };
			}
			case D: {
				return new VoronoiCell[] { c, b };
			}
			default:
				throw new IllegalArgumentException("Invalid vertex ordinal");
			}
	...
	private class FaceDAC extends OrientedFace {
		...
		public VoronoiCell[] getEdge(VoronoiCell v) {
			switch (ordinalOf(v)) {
			case D: {
				return new VoronoiCell[] { a, c };
			}
			case A: {
				return new VoronoiCell[] { c, d };
			}
			case C: {
				return new VoronoiCell[] { d, a };
			}
			default:
				throw new IllegalArgumentException("Invalid vertex ordinal");
			}
	 ...
	 * Vertex C
	 */
	private VoronoiCell c;

	/**
	 * Vertex D
	 */
	private VoronoiCell d;
	...
	public void addFaces(List<VoronoiCell[]> faces) {
		faces.add(new VoronoiCell[] { a, d, b });
		faces.add(new VoronoiCell[] { b, c, a });
		faces.add(new VoronoiCell[] { c, b, d });
		faces.add(new VoronoiCell[] { d, a, c });
	}
	...
	public VoronoiCell[] getVertices() {
		return new VoronoiCell[] { a, b, c, d };
	}
	...
	public Point3d[] getVerticesPoint3d() {
		return new Point3d[] { a.asPoint3d(), b.asPoint3d(), c.asPoint3d(), d.asPoint3d() };
	}
	 ...
	 * @param vNew
	 */
	void patch(VoronoiCell old, Tetrahedron n, V vNew) {
	 ...
	 * @param face
	 */
	void traverseVoronoiFace(Tetrahedron origin, Tetrahedron from, VoronoiCell vC,
			VoronoiCell axis, List<Point3d> face) {
		if (origin == this) return;
		double[] center = new double[3];
		de.grogra.math.util.Geometry.centerSphere(
				a.getX(), a.getY(), a.getZ(), b.getX(), b.getY(), b.getZ(), 
				c.getX(), c.getY(), c.getZ(), d.getX(), d.getY(), d.getZ(), center);
		face.add(new Point3d((float) center[0], (float) center[1], (float) center[2]));
	 ...
	 * @param face
	 */
	void traverseVoronoiFace(VoronoiCell vC, VoronoiCell axis, List<Point3d[]> faces) {
		ArrayList<Point3d> face = new ArrayList<Point3d>();
		double[] center = new double[3];
		de.grogra.math.util.Geometry.centerSphere(a.getX(), a.getY(), a.getZ(), b.getX(), b.getY(), b.getZ(), c.getX(), c.getY(), c.getZ(), d.getX(), d.getY(),
				d.getZ(), center);
		face.add(new Point3d((float) center[0], (float) center[1],
				(float) center[2]));
		V v = VORONOI_FACE_ORIGIN[ordinalOf(vC).ordinal()][ordinalOf(axis)
				.ordinal()];
	 ...
	 */
	void visitStar(VoronoiCell vC, StarVisitor visitor) {
	...
	void visitStar(VoronoiCell vC, StarVisitor visitor,
		...
		if (visited.add(this)) {
			...
			case D:
				...
				break;
			default:
				throw new IllegalArgumentException("Invalid center vertex: "
						+ vC);
			}
		}
	}
